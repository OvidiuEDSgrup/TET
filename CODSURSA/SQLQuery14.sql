...
set aprobat_alte = isnull((select sum(case when p.cant_aprobata>(case when p.punct_livrare='' then p.cant_realizata else p.pret_promotional end) then p.cant_aprobata-(case when p.punct_livrare='' then p.cant_realizata else p.pret_promotional end) else 0 end) 
 from pozcon p where p.subunitate='1        ' and p.tip='BK'  and (select max(stare) from con where subunitate = p.subunitate and tip = p.tip and contract = p.contract and data = p.data and tert = p.tert)<='1' and p.cod=comlivrtmp.cod and (0=0 or p.factura='101      ')), 0)
where utilizator='OVIDIU'
