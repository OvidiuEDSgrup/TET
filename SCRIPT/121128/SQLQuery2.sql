...
--/*startsp  
(select convert(decimal(15,2),sum(round(convert(decimal(17, 5), p.pret/(1+case when gp.tip_gestiune in ('A','V') then p.cota_tva else 0 end/100)  
...
where p.Subunitate=d.Subunitate and p.Tip=d.Tip and p.Contract=d.Contract and p.Data=d.Data and p.Tert=d.Tert) as valoare,  
--stopsp*/convert(decimal(15,2),d.total_contractat) as valoare,  
--/*startsp  
(select convert(decimal(15,2),sum(round(round(convert(decimal(17, 5), p.pret/(1+case when gp.tip_gestiune in ('A','V') then p.cota_tva else 0 end/100)  
   *(1-p.discount/100)*(1-isnull(pe.pret,0)/100)*(1-isnull(pe.cantitate,0)/100))  
   *p.Cota_TVA/100,2)*p.cantitate,2)))  
from pozcon p  
 left outer join pozcon pe on pe.Subunitate='EXPAND' and pe.Tip=p.Tip and pe.Contract=p.Contract and pe.Tert=p.Tert and pe.Data=p.Data and pe.Cod=p.Cod        
 left outer join gestiuni gp on gp.cod_gestiune = p.Punct_livrare  
where p.Subunitate=d.Subunitate and p.Tip=d.Tip and p.Contract=d.Contract and p.Data=d.Data and p.Tert=d.Tert) as valtva,  
--stopsp*/convert(decimal(15,2),d.total_tva) as valtva,    
convert(decimal(15,2),  
--/*startsp  
(select convert(decimal(15,2),sum(round(convert(decimal(17, 5), p.pret*(1+(p.cota_tva-case when gp.tip_gestiune in ('A','V') then p.cota_tva else 0 end)/100)  
   *(1-p.discount/100)*(1-isnull(pe.pret,0)/100)*(1-isnull(pe.cantitate,0)/100))  
   *p.cantitate,2)))  
from pozcon p  
 left outer join pozcon pe on pe.Subunitate='EXPAND' and pe.Tip=p.Tip and pe.Contract=p.Contract and pe.Tert=p.Tert and pe.Data=p.Data and pe.Cod=p.Cod        
 left outer join gestiuni gp on gp.cod_gestiune = p.Punct_livrare  
where p.Subunitate=d.Subunitate and p.Tip=d.Tip and p.Contract=d.Contract and p.Data=d.Data and p.Tert=d.Tert)  
--stopsp*/d.total_contractat+d.total_tva  
) as valtotala,   
...
d.stare + '-' + (case when isnull(pa.val_alfanumerica, '')<>'' then rtrim(pa.val_alfanumerica) else (case d.Stare when '0' then 'Operat' when '1' then 'Definitiv' when '2' then 'Blocat' when '3' then 'Confirmat' when '4' then 'Expediat' when '5' then 'In 
vama' when '6' then 'Realizat' when '7' then 'Reziliat' else d.stare end) end) as denstare,   
