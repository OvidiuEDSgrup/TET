...
begin  
 ...
 declare @subunitate varchar(9), @searchText varchar(80), @userASiS varchar(10),   
   @lista_gestiuni bit, @tip varchar(2),@faraRestrictiiProp int  
 ...
 select @folosinta=(case when ( ((@raport in ('Balanta stocuri','Stocuri la data') or @raport like '%folosinta%') and @tiprap='F') or (@meniu='DO' and @tip='CI')) then 1 else 0 end),  
   ...
   @cuImobilizari=(case when @raport <>'' and @cale<>''  
          and (charindex(' MF ',@cale)>0 or charindex('/MF/',@cale)>0) then 2  
            --> pentru rapoartele de Mf se iau si gestiuni de tip imobilizari  
         when @tip in ('MI', 'MT') then 1 when @tip not in ('MI', 'MT') then 0 end)  
 ...
 rtrim(case when gestiuni.Cont_contabil_specific='' then   
  (case gestiuni.Tip_gestiune when 'M' then 'Materiale' when 'P' then 'Produse' when 'C' then 'Cantitativa' when 'A' then 'Amanuntul' when 'V' then 'Valorica' when 'O' then 'Obiecte' when 'F' then 'Folosinta' when 'I' then 'Imobilizari' else gestiuni.Tip_
gestiune end)  
 ...
 where subunitate=@subunitate and @folosinta=0 and  
 ...
 -- mai jos am corectat in: gestiuni tip I daca intrari/transferuri de MF, in rest (deci nu MF): gestiuni tip ne I  
 and (@raport<>'' or @tip in ('MI', 'MT') and Tip_gestiune='I' or @tip not in ('MI', 'MT') and Tip_gestiune<>'I')  
