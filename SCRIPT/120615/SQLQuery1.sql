select * from pozdoc p where 1=1
--AND p.Tip='te'
AND p.cod='10800044' 
...
,(select MAX(contract) 
from stocuri s where s.Cod=te.Cod and s.Cod_intrare=te.Grupa and s.Cod_gestiune=te.Gestiune_primitoare and s.Stoc>0) as rezervare
,*
...
and exists 
	(select 1 from pozdoc ie where ie.Tip_miscare='E' and ie.Gestiune=te.Gestiune_primitoare and ie.Cod=te.Cod 
		and ie.Cod_intrare=te.Grupa	and ie.Contract=te.Factura)

select * from sysspd s order by s.Data_stergerii desc

select
(select MAX(contract) 
from stocuri s where s.Cod=p.Cod and s.Cod_intrare=p.Grupa and s.Cod_gestiune=p.Gestiune_primitoare and s.Stoc>0) as rezervare 
,(select MAX(contract) 
from istoricstocuri s where s.Cod=p.Cod and s.Cod_intrare=p.Grupa and s.Cod_gestiune=p.Gestiune_primitoare and s.Stoc>0) as rezistoric 
,p.*
...
where s.Data_stergerii='2012-06-01 16:23:11.130' and s.Cod='10800044' 

select * from stocuri s where s.Cod_gestiune='300' and s.Stoc=0
AND S.Cod LIKE '0007001'

select * from sysspd s where s.Stergator='ASIS' order by s.Data_stergerii desc