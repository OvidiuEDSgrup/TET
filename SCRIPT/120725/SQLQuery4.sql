...
declare @cod varchar(20)='',@comanda varchar(20)='2840121245037'

select * from pozdoc p where p.Subunitate='1' and p.Tip='TE' and p.Gestiune_primitoare='700' 
and p.Comanda<>''
and exists 
...
--select * from pozdoc p where p.Subunitate='1' and p.Tip='TE' and p.Gestiune='700' 
--and p.Comanda<>''
--and exists 
--(select 1 from stocuri s where s.Subunitate=p.Subunitate and s.Cod_gestiune=p.Gestiune and s.Cod=p.Cod and s.Cod_intrare=p.Cod_intrare
--and s.Comanda<>p.Comanda)
--and (isnull(@comanda,'')='' or p.Comanda=@comanda)
--and (isnull(@cod,'')='' or p.cod=@cod)
