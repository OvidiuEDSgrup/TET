...
fo13_pozitii, isnull(info14_pozitii, '') as info14_pozitii, isnull(info15_pozitii, '') as info15_pozitii, isnull(info16_pozitii, '') as info16_pozitii, isnull(info17_pozitii, '') as info17_pozitii,   
 ...
 begin   
  ...
  if @tip in ('BF') and @lCuTermene=1 -- se presupune lucrul pe termene, altfel se va merge pe "else"  
  begin  
   if (@tert='' or @tert is null)   
    raiserror('Nu se poate adauga contract fara completare tert!', 11, 1)  
   if @subtip='BA'   
   ...
   if isnull(@Tpret,0)=0--daca nu s-a introdus pret se ia pretul din nomenclator  
   ...
   if not exists (select 1 from pozcon where subunitate=@sub and tip=@tip and contract=@contract and tert=@tert   
    ...
    and /*modificat sa adauge pozitie noua dc are explicatii diferite pt APE-Andrey*/ Explicatii=@explicatii_pozitii)  and @update='0'  
    insert pozcon   
   ...
   else /* Modificare antet termene :sursa, pret pe cod daca cantitate realizata=0, nu a fost facturat*/  
    if @update='1' and @subtip='BF'  
     if exists(select 1 from pozcon where subunitate=@sub and contract=@contract and tert=@tert   
      and data=@data and (cod=@cod or cod=@o_cod)and Cant_realizata=0)--daca pozitia nu a fost facturata se fac modificari  
     begin       
     ...
     else --de pe aceasta pozitie s-a facturat, se returneaza mesaj de eroare  
   ...
   if @subtip='GT' --subtipul de generare pozitie si termene aferente cu periodicitate(lucnat, bilunar...etc)  
   begin  
    set @nr=0   
   ...
   else -- subtip BF, subtip de modificare cantitati lunare pe termene(campuri pt fiecare luna din an)  
   begin  
...
,Contract,tert,data,cod),   
   ...
   --reglare cantitate din pozcon, conform modificarilor din termene  
   update pozcon set  cantitate=isnull((select SUM(round(cantitate,5)) from Termene where subunitate=@Sub and tip=@tip   
   ...
   --reglare cantitate din con conform cantitati din pozcon    
  ...
  else -- BK,FA,FC, sau si BF daca nu se lucreaza cu termene  
  begin    
   ...
   begin  
    ...
    select @Sub, @tip, @contract, @tert, @gestiune_primitoare, @data,   
     ...
     (case when @CantAprob0BKBP=1 and @tip in ('BK', 'BP') then 0 else @cantitate end), 0, @valuta, @cota_TVA,   
