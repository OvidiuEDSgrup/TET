...
select rtrim(te.subunitate) as subunitate, convert(varchar(10),te.termen,103) as codsisursa, te.cantitate as Taugust,    
  ...
  convert(decimal(17, 5), p.cantitate-(case when isnull(n.UM_1, '')<>'' and isnull(n.Coeficient_conversie_1, 0)<>0 then floor((p.cantitate-(case when isnull(n.UM_2, '')<>'' and isnull(n.Coeficient_conversie_2, 0)<>0 then floor(p.Cantitate/n.Coeficient_con
versie_2) else 0 end))/n.Coeficient_conversie_1) else 0 end)-(case when isnull(n.UM_2, '')<>'' and isnull(n.Coeficient_conversie_2, 0)<>0 then floor(p.Cantitate/n.Coeficient_conversie_2) else 0 end)) as cantitateum1,      
...
se 0 end))/n.Coeficient_conversie_1) else 0 end)) as cantitateum2,      
  ...
  ,convert(decimal(17, 5), p.pret*(1-p.discount/100)/(1+case when gest.tip_gestiune in ('A','V') then p.cota_tva else 0 end)/100) as pretvanzare  
  ,convert(decimal(15,2),round(p.cantitate*convert(decimal(17, 5), p.pret*(1-p.discount/100)/(1+case when gest.tip_gestiune in ('A','V') then p.cota_tva else 0 end)/100),2)) as valoare  
  ,convert(decimal(15,2),round(round(p.Cantitate*convert(decimal(17, 5), p.pret*(1-p.discount/100)/(1+case when gest.tip_gestiune in ('A','V') then p.cota_tva else 0 end)/100)*p.Cota_TVA/100,2)*p.cantitate,2)) as valtva  
  ,convert(decimal(15,2),round(p.cantitate*convert(decimal(17, 5), p.pret*(1-p.discount/100)/(1+case when gest.tip_gestiune in ('A','V') then p.cota_tva else 0 end)/100),2)  
   +round(round(p.Cantitate*convert(decimal(17, 5), p.pret*(1-p.discount/100)/(1+case when gest.tip_gestiune in ('A','V') then p.cota_tva else 0 end)/100)*p.Cota_TVA/100,2)*p.cantitate,2)) as valtotala  
  /*stopsp*/  
into #tmppozcon    
from pozcon p        
left outer join nomencl n on n.cod = p.Cod         
...
/*startsp*/  
left outer join gestiuni gprim on gprim.cod_gestiune = p.Punct_livrare  
/*stopsp*/  
