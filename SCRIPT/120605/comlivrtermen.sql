...
SET @data1='2011-12-01'
SET @data2='2012-03-31'
SET @tert=null
SET @comanda=null
SET @stare='1,2,3'
SET @bifa='D'
...
INTO #cantComTermen
FROM Termene t
WHERE t.Subunitate=@Subunitate and t.Tip=@Tip and t.Termen between @data1 and @data2 and t.Cantitate>0
...
left join #cantComTermen ct ON ct.Subunitate=p.Subunitate and ct.Tip=p.Tip and ct.Contract=p.Contract and ct.Tert=p.Tert and ct.Cod=p.Cod
	and ct.data=p.Data
where p.subunitate = @Subunitate and p.tip =@Tip and c.stare <= '1' and p.cantitate>0 and n.tip not in ('R', 'S')
...
and ISNULL(ct.termen,p.termen) between @data1 and @data2
