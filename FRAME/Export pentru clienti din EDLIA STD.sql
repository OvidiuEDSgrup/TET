...
-- se separa prin 'GO' fiecare tabela
/*
	exec exportMeniuSTDRia @tipMacheta='C', @meniu='G', @tip='', @subtip=''
	GO
*/
--***
if exists (select * from sysobjects where name ='exportMeniuSTDRia')
drop procedure exportMeniuSTDRia
go
--***
create procedure exportMeniuSTDRia @tipMacheta varchar(20),@meniu varchar(20), @tip varchar(20), @subtip varchar(20),
...
begin
	...
	from webConfigSTDMeniu w where (@meniu is null or meniu=@meniu)
end
...
from webconfigSTDGrid w left join webconfigSTDmeniu m on w.meniu=m.meniu where (@meniu is null or w.meniu=@meniu) and (w.tip=@tip or @tip='') 
and (w.subtip=@subtip or @subtip='')
...
begin
	...
	from webconfigSTDFiltre w where (@meniu is null or w.meniu=@meniu) and (w.tip=@tip or @tip='')
end
...
from webconfigSTDTipuri w where (@meniu is null or w.meniu=@meniu) and (w.tip=@tip or @tip='') and (w.subtip=@subtip or @subtip='')
...
begin
	...
	begin
		...
		exec exportMeniuSTDRia @tipMacheta=@tipMachetaNou, @meniu=@meniuNou, @tip=@tipNou,@subtip='', @text = @tmpRez output
