...
as 
begin
	declare @bugetari int, @modimpl int, @rulajepelm int, @datal datetime, @tip varchar(2), @subtip varchar(2), 
		@sub varchar(9), @numar varchar(8), @data datetime, @datapf datetime, @nrinv varchar(13), @contmf varchar(40), 
		@contcor varchar(40), @contcham varchar(40), @numardvi varchar(30), @mesaj varchar(255), @ptupdate int
...
--	set @sub=isnull(@document.value('(/row/row/@sub)[1]', 'varchar(9)'),'')
	...
	set @datapf=isnull(@document.value('(/row/row/@datapf)[1]', 'datetime'), '01/01/1901')
	set @nrinv=isnull(@document.value('(/row/row/@nrinv)[1]', 'varchar(13)'), isnull(@document.value('(/row/@nrinv)[1]', 'varchar(13)'), ''))
	set @numardvi=isnull(@document.value('(/row/row/@numardvi)[1]', 'varchar(30)'),'')
	set @ptupdate=isnull(@document.value('(/row/row/@update)[1]', 'int'), 0)

	if @tip='MI' set @contmf=isnull(@document.value('(/row/row/@contmf)[1]', 'varchar(40)'), '')
	...
	set @contcor=isnull(@document.value('(/row/row/@contcor)[1]', 'varchar(40)'), '')
	set @contcham=@document.value('(/row/row/@contcham)[1]', 'varchar(40)')

	if @tip='MM' and @ptupdate=0
		set @datapf=isnull((select Data_punerii_in_functiune from mfix where Numar_de_inventar=@nrinv and Subunitate=@sub),@datapf)
	...
	if @document.exist('/row/row')=1 and (@datapf < dbo.bom(@datal) or @datapf > dbo.eom(@datal)) 
	and @tip in ('MI') and @subtip not in ('SU','DO','AL')
	begin
		raiserror('Data punerii in functiune nu este in luna aleasa!',11,1)
		return -1
	end

	if @document.exist('/row/row')=1 and @datapf > @data and @tip in ('MM') 
	begin
		raiserror('Data punerii in functiune ulterioara datei modificarii!',11,1)
		return -1
	end
	...
	if @document.exist('/row/row')=1 --and isnull(@document.value('(/row/row/@codcl)[1]', 'varchar(20)'), '') = ''
	and @tip in ('MI')
	and not exists (select 1 from codclasif where 
	cod_de_clasificare=isnull(@document.value('(/row/row/@codcl)[1]', 'varchar(20)'), ''))
	begin
	...
	and @tip in ('MI') and isnull(@document.value('(/row/row/@pretvaluta)[1]', 'float'), 0) = 0
	begin
	...
	and @subtip in ('AF','FF','VI','CS') and (@subtip<>'AF' or @numardvi='')
	begin
	...
	if @document.exist('/row/row')=1 --and isnull(@document.value('(/row/row/@contgestprim)[1]', 'varchar(40)'), '') = ''
	and @tip in ('MT')
	and not exists (select 1 from gestiuni where tip_gestiune='I' and 
	cod_gestiune=isnull(@document.value('(/row/row/@contgestprim)[1]', 'varchar(40)'), ''))
	begin
	...
	if @document.exist('/row/row')=1 --and isnull(@document.value('(/row/row/@contlmprim)[1]', 'varchar(40)'), '') = ''
	and @tip in ('MT') and not exists (select 1 from lm where 
	cod=isnull(@document.value('(/row/row/@contlmprim)[1]', 'varchar(40)'), '') and nivel in (select 
	strlm.nivel from strlm where mijloace_fixe=1))
	begin
	...
	if @document.exist('/row/row')=1 --and isnull(@document.value('(/row/row/@contamcomprim)[1]', 'varchar(40)'), '') = ''
	and (@tip='MI' or @tip='MM' and @subtip='MF') and not exists (select 1 from conturi where 
	cont=isnull(@document.value('(/row/row/@contamcomprim)[1]', 'varchar(40)'), '') and are_analitice=0)
	begin
		raiserror('Cont amortizare inexistent sau cu analitice!',11,1)
		return -1
	end

	if @tip='MI' and (@bugetari=1 or @contcham<>'')
		and not exists (select 1 from conturi where cont=@contcham and are_analitice=0)
	begin
		raiserror('Cont cheltuieli amortizare inexistent sau cu analitice!',11,1)
		return -1
	end
	
	if @document.exist('/row/row')=1 --and isnull(@document.value('(/row/row/@conttva)[1]', 'varchar(40)'), '') = ''
	and isnull(@document.value('(/row/row/@sumatva)[1]', 'float'), 0) <> 0
	and @tip in ('MI')
	and @subtip in ('AF')
	and not exists (select 1 from conturi where 
	cont=isnull(@document.value('(/row/row/@conttva)[1]', 'varchar(40)'), '') and are_analitice=0)
	begin
	...
	if @document.exist('/row/row')=1 and isnull(@document.value('(/row/row/@update)[1]', 'int'), 0)=1
	and isnull(@document.value('(/row/row/@contlmprim)[1]', 'varchar(40)'), '') <> 
	isnull(@document.value('(/row/row/@o_contlmprim)[1]', 'varchar(40)'), '') 
	...
	if @document.exist('/row/row')=1 --and isnull(@document.value('(/row/row/@contcor)[1]', 'varchar(40)'), '') = ''
	...
	if @document.exist('/row/row')=1 and right(@tip,1)+@subtip in ('ECS') 
		and exists (select 1 from mfix where subunitate='DENS' and numar_de_inventar=@nrinv and tip_amortizare='1')
	begin
		raiserror('Nu se poate face casare pentru un numar de inventar aflat in patrimoniu "Public"! Treceti numarul de inventar in patrimoniu "Privat" si apoi faceti casarea!',11,1)
		return -1
	end
	
	/*	Validare conturi la modificare tip patrimoniu */
	if @document.exist('/row/row')=1 and right(@tip,1)+@subtip in ('MTP') 
		and (nullif(@document.value('(/row/row/@contpatrimiesire)[1]','varchar(40)'),'') is not null 
				and left(nullif(@document.value('(/row/row/@contpatrimiesire)[1]','varchar(40)'),''),3) not in ('101','102')
			or nullif(@document.value('(/row/row/@contpatrimintrare)[1]','varchar(40)'),'') is not null 
				and left(nullif(@document.value('(/row/row/@contpatrimintrare)[1]','varchar(40)'),''),3) not in ('101','102'))
	begin
		raiserror('Contul de iesire/intrare din patrimoniu trebuie sa fie 101, 102!',11,1)
		return -1
	end
	
	/*	Validare la MRE, ca valoarea amortizata sa fie mai mare sau egala cu Valoarea amortizata pe clasa 8 */
	if @document.exist('/row/row')=1 and right(@tip,1)+@subtip in ('MRE') 
		and isnull(@document.value('(/row/row/@pret)[1]','float'),0)<isnull(@document.value('(/row/row/@sumatva)[1]','float'),0) and isnull(@document.value('(/row/row/@sumatva)[1]','float'),0)<>0
	begin
		raiserror('Valoarea amortizata trebuie sa fie mai mare/egala ca valoarea amortizata pe clasa 8!',11,1)
		return -1
	end
end
