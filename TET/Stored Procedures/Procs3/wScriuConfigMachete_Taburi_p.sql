--***
create procedure wScriuConfigMachete_Taburi_p (@sesiune varchar(50), @parXML xml)
as
...
begin
	...
	if @meniu<>''
		set @populare.modify('insert(attribute b_meniu_sursa {sql:variable("@meniu")}) into (/row[1])')
	if @tip<>''
		set @populare.modify('insert(attribute b_tip_sursa {sql:variable("@tip")}) into (/row[1])')
	select @populare
end
