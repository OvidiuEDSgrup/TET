create procedure [dbo].[yso_wOPIncasareStornare](@sesiune varchar(50), @parXML xml) as             
begin try           
	set transaction isolation level read uncommitted
	--begin tran yso_wOPIncasareStornare
	...
	set @formular=ISNULL(nullif(@parXML.value('(/*/@formular)[1]', 'varchar(13)'),''), 'CHITRAP')
...
END--SP*/
	...
	if isnull(@suma,0)=0
		select @suma=SUM(Pret_cu_amanuntul)
		...
		set @docRE=
			(SELECT
				...
				(select
					...
					'Stornare factura '+ @factura+ ' din data '+@datafacturii explicatii, @tert tert, @factura factura
				for XML raw, TYPE)
			for xml RAW)
...
--/*SP	-------------------------------Generare formular daca e pusa bifa-----------------------------------------
	   ...
	   SELECT 'S-a generat cu succes documentul AP'+RTRIM(@numar)+' din data de  '+LTRIM(CONVERT(VARCHAR(20),@data,103))
			+' de plata stornare pe contul '+rtrim(@contcasa) AS textMesaj, 'Info' as titluMesaj  for xml raw, root('Mesaje') 
	end        
		
	--commit tran yso_wOPIncasareStornare
end try 
begin catch 
	IF EXISTS (SELECT 1 FROM sys.dm_tran_active_transactions WHERE name = 'yso_wOPIncasareStornare')            
		ROLLBACK TRANSACTION yso_wOPIncasareStornare
    declare @eroare varchar(200) 
	set @eroare='yso_wOPIncasareStornare: '+ERROR_MESSAGE()
