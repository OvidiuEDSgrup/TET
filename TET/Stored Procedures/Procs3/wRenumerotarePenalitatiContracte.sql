--***
create procedure wRenumerotarePenalitatiContracte @sesiune varchar(50),@parXML xml 
as     
-- apelare procedura specifica daca aceasta exista.
if exists (select 1 from sysobjects where [type]='P' and [name]='wRenumerotarePenalitatiContracteSP')
begin 
	declare @returnValue int -- variabila salveaza return value de la procedura specifica
	exec @returnValue = wRenumerotarePenalitatiContracteSP @sesiune ,@parXML output
	return @returnValue
end
...
begin try 		
	...
	select numar, tert, data 
	into #pozdoc 
	from pozdoc 
	where cod_intrare = 'APBK' and subunitate = '1' and tip = 'AS' 
		and data =  @datas and cod=@cod and left(numar,2)='P#'

	declare numardoc_ramase cursor for 
	select numar, tert, data
	from #pozdoc
	order by tert
	...
	begin
		update penalizarifact set factura=@numar_aviz 
		from pozdoc p 
	...
	update penalizarifact set factura='S'+factura 
		where left(factura,2)='P#' and data_penalizare=@datas

	delete from facturi 
		where left(factura,2)='P#'
		
	delete from doc 
	where numar_dvi='Dobanzi' and tip='AS' and data= @datas and left(factura,2)='P#'
end try
begin catch
	set @mesaj=ERROR_MESSAGE() + ' '+@numar_aviz
end catch
