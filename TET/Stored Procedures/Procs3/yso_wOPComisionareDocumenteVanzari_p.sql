...
BEGIN TRY
	...
	select f.Subunitate as sub,rtrim(f.tip) as tip,f.Data as data, rtrim(f.Numar) as numar
		...
		isnull(nullif(nullif(isnull((case f.Tip when 'AP' then pa.Data_fact when 'AC' then b.Data_facturii end),f.Data_facturii),'1900-01-01'),'1900-01-01'),f.data) as data_factura,f.Data_scadentei,
