...
declare @tip char(2), @contract char(20), @data datetime, @gestiune char(9), @gestiune_primitoare char(13), 
	...
	@T9 float,@T10 float,@T11 float,@T12 float,@jurnal varchar(20),@detalii xml,  @docDetalii xml, @fara_luare_date char(1), 
...
begin try
	...
	begin 
		...
		begin
			...
			if @gestiune_primitoare='' and (@gestiune='' or @gestiune<>@gestProprietate) and @tert=''
		...
		begin
			...
			set @dXML.modify ('insert attribute categoriePret {sql:variable("@categ_pret")} into (/row)[1]')
			...
			set @dXML.modify ('insert attribute valuta {sql:variable("@valuta")} into (/row)[1]')
			...
			--pretul se ia cu procedura wIaPreturi
			create table #preturi(cod varchar(20),nestlevel int)
			insert into #preturi
			select @cod ,@@NESTLEVEL

			exec CreazaDiezPreturi
			exec wIaPreturi @sesiune=@sesiune,@parXML=@dXML

			select /*@pret=case when @iaupretamanunt=0 then pret_vanzare else pret_amanunt end ,
				@discount=discount*/
				@pret=(case when ISNULL(@pret,0)=0 then (case when @iaupretamanunt=0 then pret_vanzare else pret_amanunt end) else @pret end ),
		...
		begin
			...
			if exists(select 1 from con where Contract=@contract and tip=@tip and tip='BF' /*and (YEAR(data)=YEAR(@data) or @tip='BF')*/) 
			...
			select @Sub, @tip, @contract, @tert, @punct_livrare, @data, '0', @lm, @gestiune, @termen, @scadenta, 
				...
				@contractcor, @info1_antet, @info4_antet, 0, 0, 
		...
		begin
			...
			if not exists (select 1 from pozcon where subunitate=@sub and tip=@tip and contract=@contract and tert=@tert 
				...
				values (@Sub, @tip, @contract, @tert, @gestiune_primitoare, @data, @cod, 
					@cantitate, @pret, 0, @Discount, @termen, @gestiune, 0, @cant_aprobata, 0, @valuta, 
					@cota_TVA, 0, @modplata, '', 0, @explicatii_pozitii, @numar_pozitie, @userASiS, 
				...
				begin
					if 1=1 or exists(select 1 from pozcon where subunitate=@sub and contract=@contract and tert=@tert 
			...
			begin
				...
				begin
					...
					begin
						...
						if 1=1 or exists (select 1 from termene where Subunitate=@sub and tip=@tip and contract=@contract 
							and tert=@tert and data=@data and cod=(case when @TermPeSurse=0 then @cod else ltrim(str(@numar_pozitie)) end) 
		...
		begin 
			...
			begin
				...
				select @Sub, @tip, @contract, @tert, @gestiune_primitoare, @data, 
					...
					round(@pret*@Cota_TVA/100.00*@cantitate*(1.00 - @Discount / 100.00),2), @modplata, '', 
		...
		from 
			(select isnull(SUM(round(cantitate*pret*(1.00 - discount / 100.00),5)),0) as total_contractat,
		...
		from 
			(select isnull(SUM(t.cantitate*t.pret*(1.00 - p1.discount / 100.00)),0) as total_contractat,
	...
	begin
		set @docXMLIaPozContract = (select rtrim(@sub) as subunitate , rtrim(@tipGrp) tip, rtrim(@numarGrp) as numar, rtrim(@tertGrp) as tert, convert(char(10), @dataGrp, 101) as data  for xml raw)
