create procedure [dbo].[wSchimbStareAngajamenteBugetare] @sesiune varchar(50), @parXML xml  
as
begin
...
begin try    
    select 
	  ...
	  set @o_stare=(select stare from angbug where numar = @numar and indicator=@indbug and data=@data) 
         ...
         --@new_data= isnull(@parXML.value('(parametri/row/@data)[1]','datetime'),'01-01-1901'),
         --@new_datam=isnull(@parXML.value('(parametri/row/@datam)[1]','datetime'),'01-01-1901'),
         --@new_indbug= isnull(@parXML.value('(parametri/row/@indbug)[1]','varchar(20)'),''),
         --@new_compartiment= isnull(@parXML.value('(parametri/row/@compartiment)[1]','varchar(9)'),''),
         --@new_suma=isnull(@parXML.value('(parametri/row/@suma)[1]','float'),0),
         --@new_valuta = isnull(@parXML.value('(parametri/row/@valuta)[1]','char(3)'),''),
         --@new_curs = isnull(@parXML.value('(parametri/row/@curs)[1]','float'),0),
         --@new_beneficiar= isnull(@parXML.value('(parametri/row/@beneficiar)[1]','varchar(20)'),''),         
         --@new_stare= isnull(@parXML.value('(parametri/row/@stare)[1]','char(1)'),''),
         --@new_explicatii=isnull( @parXML.value('(parametri/row/@explicatii)[1]','varchar(200)'),''),
         --@new_observatii= isnull(@parXML.value('(parametri/row/@observatii)[1]','varchar(200)'),''),
         
         --@o_stare= isnull(@parXML.value('(parametri/@stare)[1]','char(1)'),''),
         --@o_new_suma=isnull(@parXML.value('(parametri/row/@o_suma)[1]','float'),0),
         --@o_indbug= isnull(@parXML.value('(parametri/row/@o_indbug)[1]','varchar(20)'),'')
 ...
 ---***********Start Modificare stare angajament bugetar*********************
   ...
   begin   
    ...
    declare @suma_disponibila4 float
	set @suma=convert(decimal(12,3),@suma)
	set @suma_disponibila4=
   	...
   	if  @suma_disponibila4<@suma
 		begin
		set @mesajeroare='Acest angajament bugetar nu mai poate primi viza CFP, intrucat suma lui ('+convert(varchar,(convert(decimal(12,3),@suma)))+') este mai mare decat suma disponibila ('+convert(varchar,@suma_disponibila4)+') pe acest indicator bugetar!!'
