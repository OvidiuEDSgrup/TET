--***
CREATE PROCEDURE wOPStornareDocument @sesiune VARCHAR(50), @parXML xml 
AS
	...
	BEGIN TRANSACTION StornareDoc
		...
		(SELECT RTRIM(d.Subunitate) AS '@subunitate', 
			...
			,(SELECT RTRIM(p.cod) AS '@cod',RTRIM(p.gestiune) AS '@gestiune',CONVERT(DECIMAL(17,5),x.cantitate_storno) AS '@cantitate'
				...
				,idLinie '@idlinie',1 AS '@returneaza_inserate'			
			FROM pozdoc p 
...
BEGIN CATCH 
	...
	SET @eroare='(wOPStornareDocument): '+ERROR_MESSAGE() 
