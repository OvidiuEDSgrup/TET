--***
create procedure [dbo].[yso_wOPModificareAntetCon_p] @sesiune varchar(50), @parXML xml 
as  
begin try
declare @tip char(2), @contract char(20), @data datetime, @gestiune char(9), @gestiune_primitoare char(13), 
	...
	@n_contract varchar(20),@n_tert varchar(13),@n_data datetime,@n_tip varchar(2)--/*sp
...
select @tip tip, rtrim(@contract) numar, convert(char(10),@data,101) data , rtrim(@gestiune) gestiune, rtrim(@gestiune_primitoare) gestprim,rtrim(@tert) tert, rtrim(@factura) factura , @termen termen_antet,
	 ...
	 rtrim(@n_tert) n_tert,convert(char(10),@n_data,101) n_data,@n_tip n_tip --/*sp
	 ,convert(decimal(5,0),@info5) info5 --sp*/
for xml raw
