...
CREATE procedure  wScriuSemnificatiiIndicator  @sesiune varchar(50), @parXML XML
as
...
set @o_semnific = rtrim(isnull(@parXML.value('(/row/row/@o_semnificatie)[1]', 'varchar(50)'), ''))
set @o_limita = rtrim(isnull(@parXML.value('(/row/row/@o_valmax)[1]', 'varchar(10)'), ''))
