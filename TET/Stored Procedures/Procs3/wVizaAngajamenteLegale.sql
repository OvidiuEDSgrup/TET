create procedure [dbo].[wVizaAngajamenteLegale] @sesiune varchar(50), @parXML xml  
as
begin
...
begin try    
    select 
         ...
         --@new_numar_ang_legal= isnull(@parXML.value('(parametri/@numar_ang_legal)[1]','varchar(8)'),''),
         --@new_contract=isnull(@parXML.value('(parametri/@contract)[1]','varchar(20)'),''),
         --@new_numar_ang_bug= isnull(@parXML.value('(parametri/@numar_ang_bug)[1]','varchar(8)'),''),
         --@new_numar= isnull(@parXML.value('(parametri/@numar)[1]','varchar(8)'),''),
         --@new_numar_ordonantare= isnull(@parXML.value('(parametri/@numar_ordonantare)[1]','varchar(8)'),''),
         --@new_mod_de_plata=isnull(@parXML.value('(parametri/@mod_de_plata)[1]','varchar(30)'),''),
         --@new_documente_justificative=isnull(@parXML.value('(parametri/@documente_justificative)[1]','varchar(200)'),''),
         --@new_data= @parXML.value('(parametri/@data)[1]','datetime'), 
         --@new_data_OP= @parXML.value('(parametri/@data_OP)[1]','datetime'),   
         @new_data_CFP= isnull(@parXML.value('(parametri/@data_CFP)[1]','datetime'),'01-01-1901'),     
         --@new_data_ordonantare= @parXML.value('(parametri/@data_ordonantare)[1]','datetime'),       
         --@new_compartiment= isnull(@parXML.value('(parametri/@compartiment)[1]','varchar(9)'),''),
         --@new_suma=isnull(@parXML.value('(parametri/@suma)[1]','float'),0),
         --@new_valuta = isnull(@parXML.value('(parametri/@valuta)[1]','char(3)'),''),
         --@new_curs = isnull(@parXML.value('(parametri/@curs)[1]','float'),0),
         --@new_beneficiar= isnull(@parXML.value('(parametri/@beneficiar)[1]','varchar(20)'),''),
         --@new_explicatii=isnull( @parXML.value('(parametri/@explicatii)[1]','varchar(200)'),''),         
         @new_observatii= isnull(@parXML.value('(parametri/@observatii)[1]','varchar(200)'),'')
         --@new_contract=isnull(@parXML.value('(parametri/@contract)[1]','varchar(20)'),''),
        
         --@o_stare= isnull(@parXML.value('(parametri/@stare)[1]','char(10)'),''),
         --@o_numar= isnull(@parXML.value('(parametri/@o_numar)[1]','varchar(8)'),''),
         --@o_new_suma=isnull(@parXML.value('(parametri/@o_suma)[1]','float'),0),
         --@o_new_numar_ang_bug= isnull(@parXML.value('(parametri/@o_numar_ang_bug)[1]','varchar(8)'),''),
         --@o_data_OP = @parXML.value('(parametri/@o_data_OP)[1]','datetime'),
         --@o_data_ordonantare= @parXML.value('(parametri/@o_data_ordonantare)[1]','datetime')
