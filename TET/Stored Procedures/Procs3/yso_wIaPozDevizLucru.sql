...
CREATE procedure [dbo].[yso_wIaPozDevizLucru] @sesiune varchar(50), @parXML XML
as
Declare @nrdeviz varchar(100), @doc xml, @cautare varchar(100) 
	...
	select 
		...
		(select top 100
			...
			convert(char(10),convert(datetime,SUBSTRING(p.Explicatii,4,3)+left(p.Explicatii,3)+SUBSTRING(p.Explicatii,7,4)),101) as datafactrefact, 
			...
			(case when (select sum(s.Stoc) from stocuri s where s.cod=nomencl.cod and s.Cod_gestiune='0201' )<p.Cantitate then '#FF0000' 
				  when p.Stare_pozitie = 0 then '#000000' 
				  when p.Stare_pozitie = 1 then '#0000FF' 
				  when p.Stare_pozitie = 2 then '#33CC33'
				  						   else '#993300' end)  as culoare
				-- stare = 0 - Neacceptat; (Negru)
				-- stare = 1 - Lucru;      (Albastru)
				-- stare = 2 - Finalizat;  (Verde)
				-- stare = 3 - Facturat;   (Maro)
			,
			convert(decimal(15,2),(select sum(s.Stoc) from stocuri s where s.cod=nomencl.cod and s.Cod_gestiune='0201' )) as stoc
