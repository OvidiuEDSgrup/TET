CREATE PROCEDURE [dbo].[yso_wIaTEBon_p] @sesiune VARCHAR(50), @parXML XML
AS

DECLARE @id INT, @cautare VARCHAR(20), @tip varchar(20), @te xml
...
set @te=(SELECT top 1
			'TE' tip, CONVERT(varchar(10), Data,101) data, RTRIM(subunitate) subunitate, rtrim(Numar) numar
		from doc where subunitate='1' and Tip='TE' and data=@dataDoc and numar=@nrDoc
		for xml raw		)

	if @te is not null
		exec wIaDoc @sesiune=@sesiune, @parXML=@te
	else
	begin
		select subunitate='1', tip='TE', numar=@nrDoc, data=CONVERT(varchar(10), @dataDoc,101)
