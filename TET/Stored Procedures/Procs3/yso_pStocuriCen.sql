--***
create procedure yso_pStocuriCen(@dDataSus datetime, @cCod char(20), @cGestiune char(20), @cCodi char(20), @GrCod int, @GrGest int, @GrCodi int, @TipStoc char(1), @cCont char(13), @cGrupa char(13), 
...
declare @docstoc table (nrcrt int ,subunitate char(9),gestiune char(20),cont char(20),cod char(20),data datetime,data_stoc datetime,cod_intrare char(20),pret float,tip_document char(2),
	numar_document char(9),cantitate float,cantitate_UM2 float,tip_miscare char(1),in_out char(1),predator char(20),jurnal char(3),tert char(13),serie char(20),pret_cu_amanuntul float,
	...
	dataG datetime, dataStocG datetime, pretG float, contG char(20), dataExpG datetime, pretAmG float, 
...
insert @docstoc
select ROW_NUMBER() over(order by (case when tip_document = 'SI' then '0' else '1' end)+(case when tip_miscare='I' and tip_document<>'AI' or tip_miscare='E' and cantitate<0 then '0' when tip_document='AI' then '1' else '2' end)
		+convert(char(8),data,112)+str(numar_pozitie))
,subunitate,gestiune,cont,cod,data,data_stoc,cod_intrare,pret,tip_document,numar_document,cantitate,cantitate_UM2,tip_miscare,in_out,predator,jurnal,tert,serie,
	...
	(case when tip_document = 'SI' then '0' else '1' end)+(case when tip_miscare='I' and tip_document<>'AI' or tip_miscare='E' and cantitate<0 then '0' when tip_document='AI' then '1' else '2' end)
		+convert(char(8),data,112)+str(numar_pozitie), 	-- SI, apoi intrari, apoi iesiri
	(case when tip_miscare='I' and tip_document<>'AI' or tip_miscare='E' and cantitate<0 then '2' when tip_document = 'SI' then '1' else '0' end)
		+convert(char(8),data,112)+str(numar_pozitie), -- iesiri, apoi SI, apoi intrari = ordinea pt. "ultima intrare"
	'01/01/2999', '01/01/2999', 0, '', '01/01/2999', 0, '', '', '', '', '', '', '',idIntrareFirma,idIntrare
from dbo.fStocuri(null, @dDataSus, @cCod, @cGestiune, @cCodi, @cGrupa, @TipStoc, @cCont, 0, @Locatie, @LM, @Comanda, @Contract, @Furnizor, @Lot, null)
...
declare @nrcrt int=0
select TOP 1 * from @docstoc 
while @@ROWCOUNT>0 --and 1=0
begin
	set @nrcrt=@nrcrt+1
select
	subunitate,
	...
	min(dataG) data, min(case when tip_miscare='I' then data_stoc else '' end) data_stoc, max(case when @GrCodi=1 then cod_intrare else '' end) cod_intrare,
	max(pretG) pret,
	...
	max(case when tip_miscare='E' then data else '' end) data_ultimei_iesiri,
	sum(round(convert(decimal(15,5), (case when tip_miscare='E' then -1 else 1 end)*cantitate), 3)) stoc,
	max(contG) cont, min('') data_expirarii, max(TVA_neexigibil) tva_neexigibil,
