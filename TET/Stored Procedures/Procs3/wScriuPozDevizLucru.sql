...
CREATE procedure  wScriuPozDevizLucru  @sesiune varchar(50), @parXML XML
as
declare @sub varchar(9), @par varchar(200), @LIFO int, @devizepemecanici int, @echipemecanici int, 
	...
	@promotie char(13), @generatie smallint, @confirmattelefonic bit, @programare int,
...
set @programare = isnull(@parXML.value('(/row/@programare)[1]', 'int'), 0)
--devauto	
...
set	@dataincepere = rtrim(isnull(@parXML.value('(/row/@dataincepere)[1]', 'datetime'), '01/01/1901'))
...
if @tipresursa='P' and @gestiune='' set @gestiune=isnull((select top 1 val_alfanumerica from par where 
	tip_parametru='SA' and parametru='GESTIUNE'),'')
...
if @valoaredeviz = 0 and @update=0 and exists (select 1 from devauto where Cod_deviz=@nrdeviz)
and (select count(*) from pozdevauto where Cod_deviz = @nrdeviz) > 0
begin
...
if @update = 0 and exists (select 1 from pozdevauto where Cod_deviz = @nrdeviz and Tip_resursa = @tipresursa and Cod = @cod)
begin
...
begin
	raiserror('Gestiune inexistenta!',11,1)
	return -1
end
...
/** Jurnalizare */
declare @docJurnal xml, @data datetime, @stareJurnal int, @explicatii varchar(200)
...
if (select top 1 Tip_resursa from pozdevauto where Cod_deviz = @nrdeviz order by Pozitie_articol desc) = 'P'
	set @explicatii = 'Adaugat piesa'
else
	set @explicatii = 'Adaugat manopera'

set @docJurnal = (select @nrdeviz as nrdeviz, @stareJurnal as stare, @data as data, @explicatii as explicatii for xml raw, type)
exec wScriuJurnalDeviz @sesiune = @sesiune, @parXML = @docJurnal

/** Updatare tabela programari cu devizul care are programarea selectata */
if @programare <> ''
begin
	update Programator
	set Deviz = rtrim(@nrdeviz)
	where Numar_curent = @programare
end
