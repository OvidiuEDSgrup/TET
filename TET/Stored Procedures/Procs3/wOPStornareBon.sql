...
CREATE PROCEDURE [dbo].[wOPStornareBon] @sesiune varchar(50), @parXML XML
AS
begin try
	declare
	...
	exec wIauNrDocFiscale @parXML=@docPlaja, @numar=@numarAP OUTPUT
	...
	SELECT 'S-a generat cu succes documentul storno tip AP '+RTRIM(@numarAP)+' din data de '+LTRIM(CONVERT(VARCHAR(20),@data,103))
		+(case when @faraGenerarePlin=0 then ' si IB de stornare pe aceeasi data' else '' end) AS textMesaj for xml raw, root('Mesaje') 
...
begin catch
	...
	set @mesaj=ERROR_MESSAGE()+ ' (wOPStornareBon)'
