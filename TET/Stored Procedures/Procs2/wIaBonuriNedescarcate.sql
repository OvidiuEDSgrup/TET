--***
create procedure wIaBonuriNedescarcate @sesiune varchar(50), @parXML xml
as
begin
declare @Sub char(9), @userASiS varchar(10),@iDoc int, 
		@filtreazaGestiuni bit, @filtreazaClienti bit, @filtreazaLM bit, @top int, @tipIncasariPeFacturi int
...
sum((1-(case when @tipIncasariPeFacturi=2 then 1 else Factura_chitanta end))*convert(decimal(15,2),d.tva)+convert(decimal(15,2), d.Total*case when Factura_chitanta=1 then 1 else (100-Discount)/100 end)) as valtotala, 
...
and (fx.casam is null or d.Casa_de_marcat like cast(fx.casam as varchar(10)) + '%')
