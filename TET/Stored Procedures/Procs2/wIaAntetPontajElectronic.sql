--***
Create procedure wIaAntetPontajElectronic @sesiune varchar(50), @parXML xml
as  
begin try
	...
	@iDoc int, @tip varchar(2), @data datetime, @datajos datetime, @datasus datetime, @marca varchar(6), @salariat varchar(50), @mesaj varchar(2000), 
	...
	rtrim(i.Loc_de_munca) as lm, 
	rtrim(lm.denumire) as denlm, 
	rtrim(f.denumire) as denfunctie, 
	isnull((select count(1) from PontajElectronic pe where pe.Marca=i.Marca and convert(char(10),pe.data_ora_intrare,101) between dbo.bom(i.Data) and i.Data),0) as nrpozitii,
	...
	rtrim(p.Loc_de_munca) as lm, 
	rtrim(lm.denumire) as denlm, 
	rtrim(f.denumire) as denfunctie, 
	isnull((select count(1) from PontajElectronic pe where pe.Marca=p.Marca and convert(char(10),pe.data_ora_intrare,101) between dbo.bom(@datasus) and @datasus),0) as nrpozitii,
	...
	where (@marca is Null or p.Marca=@marca) 
		and p.loc_ramas_vacant=0 and not exists (select marca from istpers i where i.data=@datasus and i.Marca=p.Marca and (@data is null or i.data=@data)) 
...
begin catch
	set @mesaj =ERROR_MESSAGE()+' (wIaAntetPontajElectronic)'
end catch
