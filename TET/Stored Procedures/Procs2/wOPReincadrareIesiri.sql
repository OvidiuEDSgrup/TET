...
declare 
	...
	@amLegaturiContracte bit, @amLegaturiStornare bit, @amLegaturiPozDoc bit
...
begin try
	...
	select @amLegaturiPozDoc=1 from sysobjects where name='LegaturiPozDoc'
	...
	if @amLegaturiPozDoc=1
	begin
		/** Daca se lucreaza si cu acest tabel se ia o imagine a tabelului pt. randurile afectate inainte de a se sterge **/
		IF OBJECT_ID('tempdb..#legaturiPozDocSters') IS NOT NULL
			DROP TABLE #legaturiPozDocSters
		CREATE TABLE #legaturiPozDocSters (idpozdoc1 int, idpozdoc2 int)

		delete l
		OUTPUT DELETED.idpozdoc1, DELETED.idpozdoc2
		INTO #legaturiPozDocSters (idpozdoc1, idpozdoc2)
		from LegaturiPozDoc l
		JOIN #idDeSters i on (i.idPozDoc=l.idpozdoc1 OR i.idPozDoc= l.idpozdoc2)
	end
	...
	if @amLegaturiStornare=1
	/** Re-inseram legaturile in LegaturiPozDoc-- daca exista**/
		insert into LegaturiPozDoc(idpozdoc1, idpozdoc2)
		SELECT
			isr.idPozDoc, ist.idPozDoc
		from #legaturiPozDocSters l
		JOIN #inserate isr ON isr.idPozDocVechi=l.idpozdoc1
		JOIN #inserate ist ON ist.idPozDocVechi=l.idpozdoc2
