...
create proc wIaProprietatiGrupa @sesiune varchar(250), @parXML xml
as
	
	declare @grupa varchar(20)

	select @grupa=@parXML.value('(/*/@grupa)[1]','varchar(20)')
	...
	into #propr
	from proprietati p
	JOIN catproprietati cp on p.cod_proprietate=cp.cod_proprietate and p.cod=@grupa
