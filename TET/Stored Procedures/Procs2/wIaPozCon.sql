...
declare @iDoc int, @lCuTermene int, @TermPeSurse int,@doc xml,@tip varchar(2), @sub char(9),@numar char(20),@tert char(20),@numere_pozitii varchar(max),      
	@data datetime, @cautare varchar(100), @Periodicitate int, @Chirie int  
...
set @Chirie=isnull((select top 1 Val_logica from par where tip_parametru='UC' and parametru='CHIRIE'),0)  
...
where  te.Subunitate=@sub and te.tip=@tip 
	and te.Contract=@numar and te.Data=@data and te.tert=@tert  
group by te.cod  
order by te.cod          
...
select rtrim(te.subunitate) as subunitate, convert(varchar(10),te.termen,103) as codsisursa, te.cantitate as Taugust,  
	...
	(convert(decimal(17, 5), te.cant_realizata)) as cant_realizata,rtrim(isnull(n.um, '')) as um1,  
...
select	rtrim(p.subunitate) as subunitate, rtrim(p.tip) as tip, rtrim(p.tip) as subtip, rtrim(p.contract) as numar,       
	...
	convert(varchar(10),p.termen,101) as termene, convert(varchar(10),p.termen,101) as termen, convert(decimal(14, 4), p.pret) as Tpret,  rtrim(p.tert) as tert,
	...
	rtrim(isnull(n.denumire, ''))+(case when @TermPeSurse=1 and @tip='BF' and @lCuTermene=1 then ' - ('+isnull(rtrim(p.Mod_de_plata),'')+')'+isnull(RTRIM(s.Denumire),'') else '' end) as denumire,       
	...
	rtrim(p.Explicatii) as explicatii, isnull(rtrim(ns.denumire), '') as denspecif,  p.numar_pozitie as numarpozitie, RTrim(ISNULL(pe.Explicatii, '')) as lot,    
	...
	isnull((select convert(decimal(15,2),achitat) from facturi f where f.subunitate=@sub and f.tip='F' and f.tert=tr.tert and  f.tert=@tert and f.Factura=tr.Explicatii and data=tr.Data2),0)   
					as Tachitat,  
	convert(decimal(15,2),(case when @Chirie=1 then (select sum(tt.cant_realizata*tt.tpret) from #tmptermen tt where tt.ttert=p.tert and tt.tcontract=p.Contract and tt.cod=p.cod) else p.cant_realizata*p.pret end)) as Tfacturat,  
...
from pozcon p      
	left outer join nomencl n on n.cod = p.Cod       
	left outer join surse s on s.Cod=p.Mod_de_plata      
	left outer join nomspec ns on ns.Cod_special = p.Explicatii and ns.tert = p.Tert      
	...
	left outer join pozcon pe on pe.Subunitate='EXPAND' and pe.Tip=p.Tip and pe.Contract=p.Contract and pe.Tert=p.Tert and pe.Data=p.Data and pe.Cod=p.Cod and pe.numar_pozitie=p.numar_pozitie
	left outer join pozcon pe2 on pe2.Subunitate='EXPAND2' and pe2.Tip=p.Tip and pe2.Contract=p.Contract and pe2.Tert=p.Tert and pe2.Data=p.Data and pe2.Cod=p.Cod and pe2.numar_pozitie=p.numar_pozitie     
	...
	left outer join #termene tr on tr.subunitate=p.subunitate and tr.contract=p.contract and tr.Data=p.data and tr.tert=p.tert and  
...
begin  
	...
	from pozcon where #tmppozcon.subunitate=pozcon.subunitate and #tmppozcon.tip=pozcon.tip 
		and	#tmppozcon.data=pozcon.data and #tmppozcon.numar=pozcon.contract 
		and #tmppozcon.tert=pozcon.Tert and #tmppozcon.cod=pozcon.Cod and #tmppozcon.numarpozitie=pozcon.Numar_pozitie
end  
else  
	set @areDetalii=0  

set @doc=(select *,(select * from #tmptermen ter where ter.subunitate=pz.subunitate and ter.Tcontract=pz.numar and   
            ter.Tdata=pz.data and ter.cod=pz.cod and ter.modplata=pz.modplata for xml raw, type) from #tmppozcon pz order by pz.numarpozitie desc
for xml raw
, root('Ierarhie')
)  
