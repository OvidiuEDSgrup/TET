...
create procedure wmIaAlteIesiri @sesiune varchar(50), @parXML xml as
begin try
	declare
	...
	(
		select TOP 25
			...
			'C' as tipdetalii, 'wmDetaliiDocument' as procdetalii, rtrim(d.Cod_gestiune) gestiune
		from doc  d
		left join @gestProp gp on gp.gest=d.Cod_gestiune
		left join @lmProp l on l.lm=d.Loc_munca
		where 
			d.Subunitate='1' and Tip='AE' and data between @datajos and @datasus 
	...
	select 
		'AE' as tip, 'wmScriuAntetDocument' as proc_scriere_antet, 'D5' as form_antet, 'MAE' as form_pozitie
