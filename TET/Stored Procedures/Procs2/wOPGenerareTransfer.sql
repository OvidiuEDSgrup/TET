...
CREATE PROCEDURE wOPGenerareTransfer @sesiune VARCHAR(50), @parXML XML
AS
BEGIN TRY
	if exists (select 1 from sysobjects where [type]='P' and [name]='wOPGenerareTransferSP')
	begin
		exec wOPGenerareTransferSP @sesiune = @sesiune, @parXML = @parXML output
		return
	end
...
BEGIN CATCH
	SET @mesaj = ERROR_MESSAGE() + ' (wOPGenerareTransfer)'
