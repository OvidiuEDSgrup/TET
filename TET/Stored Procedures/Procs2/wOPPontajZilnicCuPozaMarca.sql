--***
Create procedure wOPPontajZilnicCuPozaMarca @sesiune varchar(50), @parXML xml
as
...
if @card is not null
	select top 1 @marca=marca from personal where isnull(detalii.value('(/row/@observatii)[1]','varchar(200)'),'')=@card
begin try  
...
begin catch  
	...
	set @eroare='(wOPPontajZilnicCuPozaMarca) '+ERROR_MESSAGE()
