--***
CREATE procedure [dbo].wmAlegCodDispLivrare @sesiune varchar(50), @parXML xml
as
if exists(select * from sysobjects where name='wmAlegCodDispLivrareSP' and type='P')
begin
	exec wmAlegCodDispLivrareSP @sesiune, @parXML 
	return -1
end
...
if @parXML.exist('(/row/@wmNomenclator.procdetalii)[1]')=1
	set @parXML.modify('replace value of (/row/@wmNomenclator.procdetalii)[1] with "wmScriuPozDispLivrare"')                     
else           
	set @parXML.modify ('insert attribute wmNomenclator.procdetalii {"wmScriuPozDispLivrare"} into (/row)[1]') 
