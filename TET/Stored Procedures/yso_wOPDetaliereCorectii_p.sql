...
BEGIN TRY
	...
	select l.subunit as sub, 
		...
		l.cod_articol, rtrim(f.denumire) as den_articol,l.cantitate as cantitate,
		l.pret, l.pret_valuta
	into #coduri
	...
	SELECT --1 as areDetaliiXml,
		(SELECT
			row_number() over (order by p.nrp) as nrpoz,
			cod_articol , den_articol, --RTRIM(cod_articol)+' - '+LTRIM(p.denumire) as articol,
...
BEGIN CATCH
	SET @mesaj = ERROR_MESSAGE() + ' (yso_wOPDetaliereCorectii_p)'
