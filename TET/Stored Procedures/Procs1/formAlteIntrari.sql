/**
	Formularul este folosit pentru a lista Alte intrari din macheta Intrari/Iesiri -> Alte intrari. 
				@tip		-	tipul documentului
				@numar - numarul intrarii
				@data - data iesirii

**/
CREATE PROCEDURE formAlteIntrari @sesiune VARCHAR(50), @parXML XML, @numeTabelTemp VARCHAR(100)
OUTPUT AS
begin try 
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	/** Selectul principal	**/
	SELECT 
	@unitate unitate, @sediu localitate, @cui cui, @ordreg nror, @jud judet,
	...
	convert(char(15),convert(money,round((select sum(p.cantitate*p.pret_de_stoc) from #pzFiltr p  where p.numar=max(pz.numar) and p.data=max(pz.data)),2)),1) as TVal,
	...
	**/
	if exists (select 1 from sysobjects where type='P' and name='formAlteIntrariSP1')
	begin
		exec formAlteIntrariSP1 @sesiune=@sesiune, @parXML=@parXML, @numeTabelTemp=@numeTabelTemp output
	end
...
begin catch
	set @mesaj=ERROR_MESSAGE()+ ' (formAlteIntrari)'
