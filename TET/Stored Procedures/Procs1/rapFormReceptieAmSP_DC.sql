/**
	Procedura de luare a datelor pentru formularul web (rdl) "Formular receptie Amanunt"

--**/
CREATE PROCEDURE [dbo].[rapFormReceptieAmSP_DC] --*/DECLARE
...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	INSERT INTO #PozDocFiltr (		
		...
		Factura, Gestiune_primitoare, Numar_DVI, Valuta, Curs, Data_facturii, Data_scadentei, Contract,Pret_amanunt_predator
		)
	SELECT rtrim(max(p.Tip)), rtrim(max(Numar)), rtrim(p.Cod), max(Data) data, max(rtrim(p.Gestiune)), sum(Cantitate), max(Pret_valuta), max(Pret_de_stoc), 
		...
		max(Curs), Data_facturii, max(Data_scadentei), MAX(rtrim(Contract)),MAX(p.pret_amanunt_predator)
	...
	*/
	...
	--round(pz.pret_amanunt_predator,2) pret_cat,	--round(pz.pret_amanunt_predator*pz.cantitate,2) val_cat,	round(pz.pret_valuta*1.24,2) pret_cat,	round(pz.pret_valuta*pz.cantitate*1.24,2) val_cat,		a.numele_delegatului as MEMBR1,	a.eliberat as MEMBR2,	a.mijloc_de_transport AS MEMBR3,
...
begin catch
	set @mesaj = ERROR_MESSAGE()+ ' (rapFormReceptieAmSP_DC)'
	raiserror(@mesaj, 11, 1)
end catch

/*
	exec rapFormReceptieAmSP_DC @sesiune='', @tip='RM', @numar='97', @data='2013-06-30'
*/