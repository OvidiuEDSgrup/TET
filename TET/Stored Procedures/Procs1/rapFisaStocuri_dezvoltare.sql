--***
create procedure rapFisaStocuri_dezvoltare(@sesiune varchar(50)=null,	-->	parametrul sesiune nu va avea efect pana ce nu-l vom trimite catre ftert
	@dDataJos datetime, @dDataSus datetime,@cCod varchar(20)=null, @cGestiune varchar(20)=null, @cCodi varchar(20)=null, @cCont varchar(40)=null,
	@TipStocuri varchar(20),	--> ''=toate, 'M'=Materiale, 'P'=Produse, 'A'=Marfuri
	@den varchar(20)=null, @gr_cod varchar(20)=null, 
...
begin try
	...
	select @comanda_str='
		...
		create table #stocuri_grupat(
				stocidata decimal(20,6) default 0, v_stocidata decimal(20,6) default 0,	--> pentru grupari care necesita transferul stocului initial de la o linie la alta
			...
			for xml raw)
				...
				if exists(select * from sysobjects where name like "rapFisaStocuriSPAnte" and type="P")
				begin
					declare @parXML xml -- se va construi cu ce trebuie 
					exec rapFisaStocuriSPAnte @sesiune="'+isnull(@sesiune,'')+'", @parXML=@parXML
				end
	...
	--select * from #stocuri order by data
	'
...
'
	...
	--> calcul stoc initial si valoare stoc initial pentru grupari cronologice (data, luna); o grupare de acest tip o notez cu "C"
	if @grdata is not null --or @grluna is not null
	begin
		--> trebuie avute in vedere trecerea la grupari superioare diferite ale gruparii C si faptul ca soldurile pe celelalte grupari nu se calculeaza in acest mod
		declare @nivsupdata_gr varchar(max), @nivsupdata_comp varchar(max)	--> creez o lista de atribuiri/comparatii pt toate nivelele superioare gruparii C pentru a nu scrie de mai multe ori expresia cu case-uri:
		select @nivsupdata_gr=
				case when @grdata>1 then ',@grnivel1=grnivel1' else '' end +
				case when @grdata>2 then ',@grnivel2=grnivel2' else '' end +
				case when @grdata>3 then ',@grnivel3=grnivel3' else '' end +
				case when @grdata>4 then ',@grnivel4=grnivel4' else '' end
		select @nivsupdata_comp='1=1'+replace(@nivsupdata_gr,',',' and ')
				
		
		select @comanda_str=@comanda_str+'
			create clustered index index_stocuri on #stocuri_grupat(ordnivel1, ordnivel2, ordnivel3, ordnivel4, data)
			
			--update s set stocidata=stoci, v_stocidata=v_stoci from #stocuri_grupat s
			
			declare @stocidata decimal(20,6), @v_stocidata decimal(20,6), @data datetime
				,@grnivel1 varchar(200), @grnivel2 varchar(200), @grnivel3 varchar(200), @grnivel4 varchar(200)
			select @stocidata=0, @v_stocidata=0, @data=''1901-1-1'', @grnivel1='''', @grnivel2='''', @grnivel3='''', @grnivel4=''''
			update s set
				@stocidata=(case when '+@nivsupdata_comp+' then @stocidata else 0 end)
				,@v_stocidata=(case when '+@nivsupdata_comp+' then @v_stocidata else 0 end)
				,stocidata=@stocidata-(intrari-iesiri)
				,v_stocidata=@v_stocidata-(v_intrari-v_iesiri)
				,@stocidata=@stocidata+stoci+intrari-iesiri
				,@v_stocidata=@v_stocidata+v_stoci+v_intrari-v_iesiri
				,@data=data
				'+@nivsupdata_gr+'
			from #stocuri_grupat s
			'
	end
	--> construiesc gruparile aici sa nu complic raportul - de exemplu pt sold initial calculat altfel pt gruparile cronologice (data, luna) ar fi destul de crancen:
	declare @grupate_str varchar(max), @sablon_str varchar(max)
	select @sablon_str='	select [nivel] nivel, sum(stoci) stoci, sum(v_stoci) v_stoci, sum(intrari) intrari, sum(v_intrari) v_intrari, sum(iesiri) iesiri, sum(v_iesiri) v_iesiri,
			[grNivel] grNivel,
			(case when [tipnivel] in ("CI","PR") then "" else [grNivel] end) grNivelAfisata,
			[numeNivel] numeNivel,
			max(tert) tert, max(tip_document) tip_document, max(numar_document) numar_document, max(data) data, max(nume_tert) nume_tert,
			tipnivel1, tipnivel2, tipnivel3, tipnivel4,
			max(predator) predator, max(detalii1) detalii1, max(detalii2) detalii2
	from #stocuri_grupat
		group by [grNivel]
	 order by max(ordnivel1), max(ordnivel2), max(ordnivel3), max(ordnivel4), max(data), max(case when stoci<>0 then 0 when intrari<>0 then 1 else 2 end)'
/*
	'	select nivel, stocidata, v_stocidata, stoci, v_stoci, intrari, v_intrari, iesiri, v_iesiri,
			grNivel1, grNivel2, grNivel3, grNivel4,
			numeNivel1, numeNivel2, numeNivel3, numeNivel4,
...
begin catch
	select @eroare='Eroare: '+error_message()+' (rapFisaStocuri_dezvoltare)'
end catch
...
if len(@eroare)>0
begin
	select 0 stocidata, 0 v_stocidata, 0 stoci, 0 v_stoci, 0 intrari, 0 v_intrari, 0 iesiri, 0 v_iesiri,
