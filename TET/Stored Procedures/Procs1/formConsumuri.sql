/**
	Formularul este folosit pentru a lista Receptii. 

**/
CREATE PROCEDURE formConsumuri @sesiune VARCHAR(50), @parXML XML, @numeTabelTemp VARCHAR(100)
OUTPUT AS
begin try 
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	*/
	...
	SELECT
	@firma as UNITATE,
	ltrim(pz.gestiune)+' '+rtrim(g.denumire_gestiune) as gest,
	...
	left join gestiuni g on pz.gestiune = g.cod_gestiune
	left join comenzi c on pz.comanda=c.Comanda	
	ORDER BY pz.cod
	...
	**/
	if exists (select 1 from sysobjects where type='P' and name='formConsumuriSP1')
	begin
		exec formConsumuriSP1 @sesiune=@sesiune, @parXML=@parXML, @numeTabelTemp=@numeTabelTemp output
	end
...
begin catch
	set @mesaj=ERROR_MESSAGE()+ ' (formConsumuri)'
