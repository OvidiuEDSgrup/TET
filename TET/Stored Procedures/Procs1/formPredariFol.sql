/**
	Formularul este folosit pentru a lista Predari in folosinta. 

**/
CREATE PROCEDURE formPredariFol @sesiune VARCHAR(50), @parXML XML, @numeTabelTemp VARCHAR(100)
OUTPUT AS
begin try 
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	SELECT rtrim(Numar), rtrim(Cod), max(Data) data, rtrim(Gestiune), sum(Cantitate), max(Pret_valuta), max(Pret_de_stoc), 
	...
	WHERE pz.subunitate = @subunitate
		...
		AND pz.data = @data
		and pz.numar = @numar
	group by pz.numar, pz.cod, pz.numar, pz.cod_intrare, pz.data, pz.gestiune
	...
	*/
	...
	ltrim(rtrim(pz.gestiune)) as PRED,
	ltrim(rtrim(pz.gestiune_primitoare)) as PRIM,
	...
	**/
	if exists (select 1 from sysobjects where type='P' and name='formPredariFolSP1')
	begin
		exec formPredariFolSP1 @sesiune=@sesiune, @parXML=@parXML, @numeTabelTemp=@numeTabelTemp output
	end
...
begin catch
	set @mesaj=ERROR_MESSAGE()+ ' (formPredariFol)'
