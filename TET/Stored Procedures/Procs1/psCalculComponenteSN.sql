--***
/**	procedura calcul componente Sn	*/
Create 
procedure psCalculComponenteSN (@datajos datetime, @datasus datetime, @marca char(6))
as 
begin 
...
select a.data, a.marca, max(b.loc_de_munca), 'H-', 0, 0, 
(100+sum((case when a.cod_comp<>'' then @nSalarReferinta*a.procent/100 else 0 end)))*max(c.ore)/@nOreLuna 
from componente a
