--***
CREATE PROCEDURE calcpenNou (@dataj datetime,@datas datetime,@tert varchar(13))
AS
...
--select 'inceput',GETDATE()	
	...
	--sterg facturile care au data scadentei < 1.07.2013, cu aceasta procedura calculez doar dupa noile modificari de lege
...
--select '#fFacturi',* from #fFacturi where factura like '2353%'
...
--select '#incasari',* from #incasari --where factura like '1895%'
	...
	having sum(d2.valoare+d2.tva)-sum(case when d2.data<=dbo.data_maxima(isnull(dbo.data_ultimului_calcul_pen(d2.tert,'P',1),'1901-01-01'),@dataj) then d2.achitat else 0 end)>=0.01
...
--select '#sold',* from #sold where factura like '1895%'
	...
	select d2.tert,d2.factura,d2.tip,d2.data,d2.numar,max(s.sold)+isnull(-sum(d1.achitat),0) as sold_pen,
		datediff(day,max(case when dbo.data_maxima(isnull(dbo.data_ultimului_calcul_pen(d2.tert,'P',1),'1901-01-01'),@dataj)>isnull(d1.data,'1901-1-1') 
			and dbo.data_maxima(isnull(dbo.data_ultimului_calcul_pen(d2.tert,'P',1),'1901-01-01'),@dataj)>s.data_scadentei then dbo.data_maxima(isnull(dbo.data_ultimului_calcul_pen(d2.tert,'P',1),'1901-01-01'),@dataj)
	...
	having d2.data>=max(case when dbo.data_maxima(isnull(dbo.data_ultimului_calcul_pen(d2.tert,'P',1),'1901-01-01'),@dataj)>isnull(d1.data,'1901-1-1') 
		and dbo.data_maxima(isnull(dbo.data_ultimului_calcul_pen(d2.tert,'P',1),'1901-01-01'),@dataj)>s.data_scadentei then dbo.data_maxima(isnull(dbo.data_ultimului_calcul_pen(d2.tert,'P',1),'1901-01-01'),@dataj) 
...
--select '#date_pen1',* from #date_pen --where factura like '1895%'
	...
	select s.tert,s.factura,'NE' as tip,@datas,'', max(s.sold)+isnull(-sum(d1.achitat),0) as sold_pen,
		...
		(case when max(ps.data_penalizare) is not null then dateadd(day,1,max(ps.data_penalizare)) when max(pf.data_penalizare) is not null then dateadd(day,1,max(pf.data_penalizare))
			else max(case when dbo.data_maxima(isnull(dbo.data_ultimului_calcul_pen(s.tert,'P',1),'1901-01-01'),@dataj)>isnull(d1.data,'1901-1-1') 
			and dbo.data_maxima(isnull(dbo.data_ultimului_calcul_pen(s.tert,'P',1),'1901-01-01'),@dataj)>s.data_scadentei then dbo.data_maxima(isnull(dbo.data_ultimului_calcul_pen(s.tert,'P',1),'1901-01-01'),@dataj)
		...
		from #incasari d1 
			...
			left join (select tert,factura_penalizata,max(data_penalizare) as data_penalizare from penalizarifact p where tip_doc_incasare='NE' and tip_penalizare='P' 
...
--select '#date_pen2',* from #date_pen --where factura like '1895%'
	...
	select max(s.tip) tip,s.tert,s.factura,max(d.tip) tipd,left(max(d.numar),8) numar,d.data,max(d.sold_pen)sold_pen,max(d.zile_pen)zile_pen,
		max(d.sold_pen*d.zile_pen*0.02/100) s_p,isnull(max(c.scadenta),30) scadenta,
		isnull(c.Contract_coresp,'') as contract_coresp,0.02 as procent_penalizare,max(rtrim(s.loc_de_munca)) as loc_de_munca,
		isnull(RTRIM(c.Punct_livrare),'') as punct_livrare
	into #p	
	from #date_pen d 
...
--select '#p', * from #p-- where factura like '1895%'
	...
	insert into #p
	select max(s.tip) tip,s.tert,s.factura,max(d.tip) tipd,left(max(d.numar),8) numar,d.data,max(d.sold_pen)sold_pen,max(d.zile_pen)zile_pen,
		max(d.sold_pen*d.zile_pen*0.02/100) s_p,30 scadenta,'',0.02,max(rtrim(s.loc_de_munca)) as loc_de_munca,'' as punct_livrare
	from #date_pen d 
		inner join #sold s on d.tert=s.tert and d.factura=s.factura
...
-- completare penalizari:
	...
	select tip, tert, '' as factura, factura as factura_penalizata, tipd, numar, data, sold_pen, @datas as data_incasare, zile_pen, s_p,'' as valuta,
		'P','P',1,d.contract_coresp,d.procent_penalizare,d.loc_de_munca,null,null,isnull(rtrim(d.punct_livrare),'')
	from #p d 
	where not exists (select 1 from penalizarifact pf where pf.factura_penalizata=d.factura and pf.tert=d.tert and pf.tip=d.tip and d.tipd=pf.tip_doc_incasare 
		and pf.nr_doc_incasare=d.numar and pf.data_doc_incasare>=d.data and left(pf.factura,2) in ('P#','SD'))
--select 'dupa insert in penalizfa',GETDATE()
	...
	if exists (select 1 from sysobjects where [type]='P' and [name]='calcpenNouSP')
		exec calcpenNouSP @dataj=@dataj,@datas=@datas,@tert=@tert
