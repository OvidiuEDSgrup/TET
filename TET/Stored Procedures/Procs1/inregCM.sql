--***
create procedure inregCM @dataj datetime, @datas datetime, @nrdoc varchar(20)=''
as 
begin
...
@subunitate char(9),@invCMprod int,@invadtvanx int,@rottvanx int, @nrzectvanx int, 
@ctadaos varchar(40), @angestctadaos int, @angrctadaos int, @cttvanx varchar(40), @angestcttvanx int, 
...
exec luare_date_par 'GE','INVERSCMP',@invCMprod output,0,''
...
delete from pozincon where subunitate=@subunitate and tip_document='CM' and numar_document 
	between RTRIM(@nrdoc) and RTRIM(@nrdoc)+(case when @nrdoc<>'' then '' else 'zzzzzzzzzzzzz' end) 
	and data between @dataj and @datas

declare tmpinregCM cursor for
...
isnull(c1.tip_cont,''), isnull(c1.sold_credit,0)
FROM pozdoc p 
...
WHERE p.subunitate=@subunitate and p.tip='CM' and p.data between @dataj and @datas 
...
open tmpinregCM
fetch next from tmpinregCM into @Sub,@Tip,@Numar,@Cod,@Data,@Gest,@Cant,@Pretvaluta,@Pretstoc,
...
begin
	...
	if 0=0
		begin
		set @ctdeb=(case when LEFT(@ctstoc,1)='8' then 'X' else @ctinterm end)
		set @ctcred=@ctstoc
		set @suma=dbo.rot_val((case when @ctinterm=@ctstoc or @ctinterm='' or left(@ctstoc,1)='8' 
			then 0 else @pretstoc*@cant end), 2)
	...
	if @tipgest in ('A','V') and left(@ctstoc,2) in ('35','37')
		...
		set @ctcred=(case when @invadtvanx=1 then (case when @tipgest in ('A','V') then @ctstoc 
			else 'XXX' end) else RTRIM(@cttvanx)+(case when @angestcttvanx=1 then '.'+rtrim(@gest) 
			else '' end) end)
		set @suma=dbo.rot_val((case when @invadtvanx=1 then 1 else -1 end)*(@cant*
			ROUND(@pretampred*@tvanx/(100+@tvanx),@nrzectvanx)), 2)
		set @sumavaluta=0 --dbo.rot_val(0, 2)
		set @expl='TVA neex. CM '+@numar
		exec scriuPozincon @subunitate=@sub, @Tip_document=@tip, @Numar_document=@numar, 
			@Data=@data, @Cont_debitor=@ctdeb, @Cont_creditor=@ctcred, @Suma=@suma, 
			@Valuta='', @Curs=0, @Suma_valuta=@Sumavaluta, @Explicatii=@expl, 
			@Utilizator=@userASiS, @Numar_pozitie=0, @Loc_de_munca=@lm, 
			@Comanda=@com, @Jurnal=@Jurnal, @note_receptii=0
		--adaos
		set @ctdeb=(case when @invadtvanx=1 then RTRIM(@ctadaos)+(case when @angestctadaos=1 then '.'+rtrim(@gest) 
			else '' end)+(case when @angrctadaos=1 then '.'+rtrim(@grupanom) else '' end) 
			when @tipgest in ('A','V') then @ctstoc else 'XXX' end) 
		set @ctcred=(case when @invadtvanx=1 then (case when @tipgest in ('A','V') then @ctstoc 
			else 'XXX' end) else RTRIM(@ctadaos)+(case when @angestctadaos=1 then '.'+rtrim(@gest) 
			else '' end)+(case when @angrctadaos=1 then '.'+rtrim(@grupanom) else '' end) end)
		set @suma=dbo.rot_val((case when @invadtvanx=1 then 1 else -1 end)*(@cant*
			(@pretampred-@pretstoc-ROUND(@pretampred*@tvanx/(100+@tvanx),@nrzectvanx))), 2)
		set @sumavaluta=0 --dbo.rot_val(0, 2)
		set @expl='Adaos CM '+@numar
		exec scriuPozincon @subunitate=@sub, @Tip_document=@tip, @Numar_document=@numar, 
			@Data=@data, @Cont_debitor=@ctdeb, @Cont_creditor=@ctcred, @Suma=@suma, 
			@Valuta='', @Curs=0, @Suma_valuta=@Sumavaluta, @Explicatii=@expl, 
			@Utilizator=@userASiS, @Numar_pozitie=0, @Loc_de_munca=@lm, 
