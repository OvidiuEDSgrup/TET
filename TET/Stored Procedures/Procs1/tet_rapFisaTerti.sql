-- adica fac intai drop si GO apoi create si GO apoi execut procedura creata la pasii anteriori cu parametrii doriti de mine pt testare

--***Aici creez procedura raportului. Orice modificari aduse la datele raportului (nu aspect sau grupare) se fac in aceasta procedura.
CREATE procedure [dbo].[tet_rapFisaTerti](@cFurnBenef varchar(1),@cData datetime,@cTert varchar(50),@cFactura varchar(50),@cContTert varchar(50),
...
if (@tipdoc='X' or @tipdoc='F')
		...
		where --(@tipdoc='X' or @tipdoc='F') and
			...
			and (@contcoresp is null or rtrim(ft.cont_coresp) like @contcoresp)
