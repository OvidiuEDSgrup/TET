create PROCEDURE [dbo].[formDispozitiePISP] @sesiune VARCHAR(50), @parXML XML, @numeTabelTemp VARCHAR(100) OUTPUT     
AS    
begin try     
	...
	select 
		...
		rtrim(isnull(pp.Nume,t.Denumire)) as Nume,
		...
		isnull(substring(pp.Copii, 1, 2),rtrim(af.Seria_buletin)) as SERIA,
		isnull(substring(pp.Copii, 4, 6),RTRIM(af.Numar_buletin)) as NR
	into #fmic
	FROM pozplin p
		...
		left outer join facturi f on f.Subunitate=p.Subunitate and f.Tip=0x46 and f.Tert=p.Tert and f.Factura=p.Factura
		left outer join anexafac af on af.Subunitate=f.Subunitate and af.Numar_factura=f.Factura
