...
CREATE procedure [dbo].[tet_rapVanzariTargetMarja] 
--*/ 
	...
	,@Doar_pachete bit=0,@Echipa nvarchar(4000)=null,@Grupa_terti nvarchar(4000)=null

/* si comenteaza aici pt teste
select @Data_doc_de_la='2012-01-01 00:00:00',@Data_doc_pana_la='2015-06-30 00:00:00'
	,@Loc_de_munca=null,@Tert='RO3714810',@Grupa_articole=null,@Cod_articol=NULL,@Doar_pachete=0,@Echipa=null
--*/as
select @Data_doc_de_la=isnull(@Data_doc_de_la,'2011-12-31'), @Data_doc_pana_la=isnull(@Data_doc_pana_la,GETDATE())
...
,coduri as
	(select p.Subunitate, p.data, p.Cod, Gestiune=convert(char(13),p.Gestiune), convert(char(20), p.Cod_intrare) as Cod_intrare
		,p.pret_intrare
		,Nivel=0
	from predari p
	union all
...
if OBJECT_ID('tempdb..#pozdocsitargete') is not null drop table #pozdocsitargete
...
select * from #pozdocsitargete pt
order by Data_lunii, data, Tip_doc, Numar_doc
