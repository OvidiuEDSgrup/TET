--***
create procedure rapFisaContTert(@sesiune varchar(50)=null, @cFurnBenef varchar(1),@cDataJos datetime,@cDataSus datetime,@cTert varchar(50)=null,@cContTert varchar(40),
...
/**2.	se creeaza tabela cu datele organizate pentru a se putea afisa in raport; se foloseste inca o tabela pentru a se mai putea filtra 
	...
	insert into #date(denumire, grupa, nrpoz, cont_corespondent, debitare, soldfactbenef, soldfactfurn, furn_benef, subunitate, tert,
		...
		inversare)
	select ft.denumire,ft.grupa,(case when @detTVA=1 then '1' else '' end) numar_pozitie, 
...
--test	select tert, data, nr_pe_luna_asc from #date order by tert,data,nr_pe_luna_asc
	...
	select max(ft.denumire),max(ft.grupa),'' numar_pozitie, '' contTVA,
...
/**3.	se trimit datele la raport	*/
	select ordonare, rtrim(denumire) denumire, rtrim(grupa) grupa, rtrim(a.cont_de_tert) as cont_factura, isnull(c.tip_cont,'B') tip_cont,
...
begin catch
	set @eroare='rapFisaContTert (linia '+convert(varchar(20),ERROR_LINE())+') '+char(10)+
				ERROR_MESSAGE()
end catch
