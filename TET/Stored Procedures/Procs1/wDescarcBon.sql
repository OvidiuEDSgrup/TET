--***
create procedure wDescarcBon @sesiune varchar(50), @parXML xml, @idrulare int=0
as
...
declare @UID varchar(50), @GestBon varchar(13),@factura varchar(20), @idAntetBon int,
	...
	@dataStart datetime, @msgEroare varchar(max), @eroareTimeout bit,@primaIncasare int

if exists (select * from sysobjects where name ='wDescarcDocPv')
	and (exists (select * from sysobjects where name ='wScriuDoc')
		or exists (select * from sysobjects where name ='wScriuDocBeta'))
begin
	exec wDescarcDocPv @sesiune=@sesiune, @parXML=@parXML, @idRulare=@idrulare
	return 0
end
...
declare @nFetchStatus int,@subunitate varchar(9),@Serii int, 
	...
	@CotaTVA float,@cotaTVAnom float, @SumaTVA float,@pretFaraDiscount float,@Disc float,@Barcod varchar(20),@TipNom char(1),
	...
	@gestPozitie varchar(30), @listaGestiuniPozitie varchar(300), @tertNeplatitorTva bit, @idPozdoc int, @idContract int,
	@rezervareStocComenzi bit, @gestRezervariComenzi varchar(200), @DetaliereBonuri bit, @tmpbon cursor, @CuTranzactii int, @total float,
	@idPozContract int, @idJurnalContract int, @xmlTrimis xml, @detaliiPoz xml, @idComandaHrc int
declare @devize table (cod_deviz varchar(20), cod varchar(20) primary key(cod_deviz,cod))
...
select	@TipDoc=isnull(a.bon.value('(/date/document/@tipdoc)[1]','varchar(2)'), (CASE WHEN a.chitanta=1 THEN 'AC' ELSE 'AP' END)),
		...
		@idComandaHrc=a.bon.value('(/date/document/@idComandaHrc)[1]','int'),
		@GestBon=rtrim(gestiune), @vanz=Vinzator, @Casa=a.Casa_de_marcat, @Data=a.Data_bon, @NrBon=a.Numar_bon,
		@factura=case when @TipDoc<>'AP' then '' else
			isnull(rtrim(a.factura), convert(VARCHAR(30), a.numar_bon)/*a.numar_bon doar la facturi vechi din PVria v1*/) end,
		...
		@CtFact=isnull(a.bon.value('(/date/document/@cont_factura)[1]','varchar(20)'),
					(case when @TipDoc='AP' and @AP418=1 then '418' else '' end)), 
		@xmlDoc=a.Bon
...
IF ISNULL(@NrDoc,'')='' -- din 2013-06 se stabileste @nrDoc in wScriuDatePV
begin	
...
select 
	...
	b.total,
	b.cantitate,
	b.cota_tva, n.cota_tva, 
	b.tva,
	b.pret,
	b.discount,
	b.codplu,
	isnull(n.tip, '') tipnomencl,
	rtrim(isnull(nullif(b.lm_real,''), isnull(a.Loc_de_munca, isnull(gestcor.loc_de_munca, '')))) lm,
	...
	(CASE WHEN @tertNeplatitorTva=0 or isnumeric(left(n.tip_echipament, 1))<>1 THEN 0 else convert(int, left(n.tip_echipament, 1)) END) AS tipTVA,
...
open @tmpbon
fetch next from @tmpbon into @Incas, @NrLin, @Cod, @AreSerii, @serieSauNrDocIncasare, @Coef, @total, @Cant, @CotaTVA, @cotaTVAnom, @SumaTVA, @pretFaraDiscount, @Disc, @Barcod, @TipNom, @LM, @comanda_asis, @contract, @bTip, @tipTVA, @gestPozitie, @idPozContract,@detaliiPoz
set @nFetchStatus=@@fetch_status

while @nFetchStatus=0
begin
	if DATEDIFF(second, @dataStart, getdate()) >= 20 and APP_NAME() not like '%Microsoft SQL Server%' 
	...
	begin
		...
		if @detaliiPoz is null -- backwards compatible
		begin
			set @detaliiPoz = @xmlDoc.query('(/date/document/pozitii/row[@nrlinie=sql:variable("@NrLin")])[1]')
			if @detaliiPoz is null set @detaliiPoz='<row />'

			select	@idPozContract = isnull(@detaliiPoz.value('/row[1]/@idPozContract','int'),0),
					@idJurnalContract = (case when isnull(@idJurnalContract,0)>0 then @idJurnalContract
						else @detaliiPoz.value('/row[1]/@idJurnalContract','int') end)
		end

		if @idJurnalContract=0
			set @idJurnalContract = null
		
		-- la sfarsit fac chem procedura care sa schimbe starea contractelor.
		if @idPozContract>0
			insert into @contracte
			select idContract 
			from PozContracte pc
			where idPozContract=@idPozContract
			and not exists (select * from @contracte c where c.idContract=pc.idContract)
		...
		*/
		...
		if @rezervareStocComenzi=1 and (isnull(@contract,'')<>''/*structuri vechi*/ or @idPozContract>0/*structuri noi*/)
		begin
		...
		*/
		...
		begin
			...
			begin
				...
				/*
					...
					daca se factureaza din comenzi si este acolo gestiune, stornam tot acolo.
				...
				*/
				...
				set @listaGestiuniStorno = (case when @TipDoc='AC' then ';'+rtrim(@GestBon)+';'
												/*@TipDoc='AP'*/
												when @gestPozitie<>@GestBon then ';'+rtrim(@gestPozitie)+';'
												else ';'+replace(@listaGestiuniPozitie, @GestBon+';', '')+';'+rtrim(@GestBon)+';' end)
				...
				*/
				...
				set @xml=(select @tipDoc as tip, '' as subtip, @NrDoc as numar, convert(char(10), @data,101) as data,
								...
								@factura factura, convert(char(10), @data,101) as data_facturii,
				...
				begin
					set @xmlTrimis =
						(select @idJurnalContract idJurnalContract, idContract idContract, GETDATE() data, 
								'Vanzare PVria ('+(case @TipDoc when 'AP' then 'Factura' when 'AC' then 'Bon' else @TipDoc end)+')' explicatii,
							(SELECT @idPozContract idPozContract, 
									PD.r.value('(@idPozDoc)[1]', 'int') idPozDoc
								FROM @xml.nodes('/Inserate/row') PD(r)
								for xml raw,type ) pozitii
						from PozContracte where idPozContract=@idPozContract
						for xml RAW, ROOT('Date'))	
					exec wScriuLegaturiContracte @sesiune=@sesiune, @parXML=@xmlTrimis
					
					if @idJurnalContract is null -- nu e cea mai buna solutie, dar aceasta procedura va fi inlocuita...
						select top 1 @idJurnalContract= idJurnal
						from LegaturiContracte l 
						where l.idPozContract=@idPozContract
						order by idJurnal desc 
				end
			...
			begin
				...
				if @codIntrareInDenumire=1 and isnull(@serieSauNrDocIncasare,'')<>'' or @GestSt is null
				begin
				...
				*/
				...
				begin
					...
					set @NrTE=@NrDoc -- numar TE trebuie sa fie = cu numar AC pt. ca la refaceri ne bazam ca este acelasi numar de document.
					set @CodIPrim=''
				...
				begin
					...
					set @xml = (select '' as subtip, @NrDoc as numar, @Data as data, @tert as tert, 
						...
						@CategP as categ_pret, @PretAm as pret_amanunt, @PValuta as pret_valuta,
							(select 'PV' as sursa for xml raw,type) detalii
						for xml raw)
					exec wScriuTE @xml output
					
					if @idPozContract>0 -- operatii de facut cand se scrie o pozitie din modulul contracte
					begin
						set @xmlTrimis =
							(select @idJurnalContract idJurnalContract, idContract idContract, GETDATE() data, 
									'Transfer PVria ('+(case @TipDoc when 'AP' then 'Factura' when 'AC' then 'Bon' else @TipDoc end)+')' explicatii,
								(SELECT @idPozContract idPozContract, 
										PD.r.value('(@idPozDoc)[1]', 'int') idPozDoc
									FROM @xml.nodes('/Inserate/row') PD(r)
								for xml raw,type ) pozitii
				...
				begin
					...
					set @idPozdoc = 0
					-- cumulare pozitii doar la AC-uri la care nu se vinde din deviz sau comanda de livrare.
					if @TipDoc='AC' and @comanda_asis='' and @contract='' and isnull(@idPozContract,0)=0 
						and EXISTS (SELECT * FROM syscolumns sc, sysobjects so WHERE so.id = sc.id AND so.NAME = 'pozdoc' AND sc.NAME = 'idPozDoc')
		...
		begin
			declare @codDeviz varchar(20)
			set @codDeviz=''
			if exists (select 1 from devauto where cod_deviz=@comanda_asis)
				set @codDeviz=@comanda_asis
		
			if @codDeviz<>''  
			begin
				declare @codManopera varchar(200)
				if @codManopera is null 
				...
				and (cod=@cod or Tip_resursa='M' and @cod=@codManopera) 
				
				-- salvez faptul ca am modificat pozitia aceasta de deviz. 
				-- daca sunt erori, anulez aceste modificari.
				if not exists (select * from @devize where cod_deviz=@codDeviz and cod=@cod)
					insert into @devize(cod_deviz, cod)
						values (@codDeviz, @cod)
			end
			-- schimb stare deviz
	...
	begin
		...
		select @contCasa = isnull(@detaliiPoz.value('(/row/@cont)[1]', 'varchar(50)'),'')

		if @contCasa='' -- daca nu este in xml, citesc din proprietati, si il salvez in xml.
		begin
			if @bTip='36' -- card de credit
				set @contCasa=rtrim(ISNULL((select valoare from proprietati where tip='UTILIZATOR' and Cod_proprietate='CONTCARD' and cod=@Vanz 
											and valoare<>''),'5114'))
			else
			if @bTip='37' -- puncte fidelizare /* doar la facturi, la bonuri se va opera manual din raport Z */
				set @contCasa=rtrim(ISNULL((select valoare from proprietati where tip='UTILIZATOR' and Cod_proprietate='CONTPUNCTE' and cod=@Vanz
											and valoare<>''),'609'))
			else
				set @contCasa=rtrim(ISNULL((select valoare from proprietati where tip='UTILIZATOR' and Cod_proprietate='CONTCASA' and cod=@Vanz
											and valoare<>''),'5311'))
			...
			update bt 
				set detalii = @detaliiPoz
			where idAntetBon=@idAntetBon and numar_linie=@nrlin
		end
...
while exists (select * from @contracte) -- pt schimbarea starii contractelor.
begin
	select top 1 @idContract = idContract from @contracte
	
	set @xml = (select @idContract idContract for xml raw)
	exec updateStareContract @sesiune=@sesiune, @parXML=@xml
	
	delete from @contracte where idContract=@idContract
end

if @idComandaHrc is not null
begin
	declare @docRest xml
	set @docRest=(select @idComandaHrc idComandaHrc for xml raw)
	exec wDefinitivareComandaRestaurant @sesiune=@sesiune, @parXML=@docRest
end
...
begin catch
	set @msgEroare = ERROR_MESSAGE()+'. (idAntetBon='+convert(varchar,isnull(@idantetbon,0))+') (wDescarcBon)'
	...
	begin
		...
		begin
			update p
				...
				from pozdevauto p
				inner join @devize d on p.cod_deviz=d.cod_deviz and (p.cod=d.cod or p.Tip_resursa='M' and d.cod=@codManopera) 
		...
		begin
			...
			(Casa_de_marcat, Factura_chitanta, Numar_bon, Numar_linie, Data, Ora, Tip, Vinzator, Client, Cod_citit_de_la_tastatura, CodPLU, Cod_produs, Categorie, UM, 
				...
				lm_real, Comanda_asis,[Contract], idAntetBon, idpozcontract)
