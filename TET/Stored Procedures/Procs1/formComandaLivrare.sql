...
CREATE PROCEDURE formComandaLivrare @sesiune VARCHAR(50), @parXML XML, @numeTabelTemp VARCHAR(100)
OUTPUT AS
...
IF EXISTS (
		...
		FROM sysobjects
		WHERE type = 'P'
			AND NAME = 'formComandaLivrareSP1'
		)
BEGIN
	EXEC formComandaLivrareSP1 @sesiune = @sesiune, @parXML = @parXML, @numeTabelTemp = @numeTabelTemp OUTPUT
END
