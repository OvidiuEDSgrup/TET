--***
Create procedure rapLichidareVenituri @dataJos datetime, @dataSus datetime, @marca char(6)=null, @locm char(9)=null, @strict int=0, @centralizat char(1)='S', @nivelcentr int=0, @alfabetic int=0
as
begin
	set transaction isolation level read uncommitted
	declare @userASiS char(10), @LmStatPlSal int, @CodBenSindicat char(13), @CodBenCONet char(13), @Subtipret int, @TipRetSind char(1), 
	@IndCondSalBaza int, @SpSpecSalBaza int, @Sp1SalBaza int, @Salubris int, @LungimeNivel int
...
--	cursor pt. date din brut
	...
	sum(B.Ore_lucrate__regie) as OreRegie, sum(B.Ore_lucrate_acord) as OreAcord, 
	...
	isnull((select sum(pj.Salar_categoria_lucrarii*pj.Ore_lucrate) 
			from pontaj pj 
			where pj.data BETWEEN @dataJos AND @dataSus and pj.marca=b.marca),0) Regie,
...
--		INNER JOIN brut B1 ON (B.marca= B1.marca and B.data= B1.data) 
	...
	B.data BETWEEN @dataJos AND @dataSus 
		AND (@marca is null OR b.marca=@marca) 
		AND (@locm is null or n.loc_de_munca like rtrim(@locm) +(case when @strict=0 then '%' else '' end))
		AND (P.Loc_ramas_vacant= 0 OR (P.Loc_ramas_vacant= 1 AND P.data_plec >= @dataJos))
		and (dbo.f_areLMFiltru(@userASiS)=0 or lu.cod is not null)
	GROUP BY b.Data, (case when @LmStatPlSal=1 then P.Loc_de_munca else N.loc_de_munca end), B.marca

--	centralizare date
	select (case when @centralizat='D' then T1.data else @datasus end) as Data, (case when @centralizat='T' then '' else LM.Cod end) as LM, 
	(case when @centralizat='T' then '' else max(LM.denumire) end) as DenLM, 
	(case when @centralizat in ('S','D') then isnull(T1.Marca, T2.Marca) else '' end) as Marca, (case when @centralizat in ('S','D') then max(P.nume)else '' end) as Nume, 
	...
	sum( round( isnull( T1.OreRegie, 0), 0)) OreRegie, sum( round( isnull( T1.OreAcord, 0), 0)) OreAcord
	from #TmpBrut AS T1		
		...
		inner join LM ON (@centralizat='T' and LM.cod=T1.Loc_de_munca or @centralizat in ('S','D') and LM.COD=isnull( T1.Loc_de_munca, T2.Loc_de_munca) 
		or @centralizat in ('N','L') and LM.COD=left(isnull( T1.Loc_de_munca, T2.Loc_de_munca), LEN( RTRIM( LM.cod))))
	WHERE (@locm is null or isnull( T1.Loc_de_munca, T2.Loc_de_munca) like rtrim(@locm)+(case when @strict=0 then '%' else '' end))
	GROUP BY (case when @centralizat='T' then '' else LM.Cod end), 
		(case when @centralizat in ('S','D') then isnull(T1.Marca, T2.Marca) else '' end), (case when @centralizat='D' then T1.data else @datasus end)
	...
	Order BY (case when @centralizat='T' then '' else LM.Cod end), 
		(case when @alfabetic=1 then max(p.Nume) else '' end), 
		(case when @centralizat in ('S','D') then isnull(T1.Marca, T2.Marca) else '' end)
...
/*
	exec rapLichidareVenituri '02/01/2015', '02/28/2015', null, '140', 0, 'S', 0, 1
*/
