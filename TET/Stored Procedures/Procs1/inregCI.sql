--***
create procedure inregCI @dataj datetime, @datas datetime, @nrdoc varchar(20)=''
as 
begin
...
delete from pozincon where subunitate=@subunitate and tip_document='CI' and numar_document 
	between RTRIM(@nrdoc) and RTRIM(@nrdoc)+(case when @nrdoc<>'' then '' else 'zzzzzzzzzzzzz' end) 
	and data between @dataj and @datas

declare tmpinregCI cursor for
...
WHERE p.subunitate=@subunitate and p.tip='CI' and p.data between @dataj and @datas 
...
open tmpinregCI
fetch next from tmpinregCI into @Sub,@Tip,@Numar,@Cod,@Data,@Gest,@Cant,@Pretvaluta,@Pretstoc,
...
begin
	...
	if 0=0
		begin
		set @ctdeb=(case when @ctinterm='' then @ctcor else @ctinterm end)
		set @ctcred=@ctstoc
		set @suma=dbo.rot_val(@cant*@pretstoc, 2)
		set @sumavaluta=0 --dbo.rot_val(0, 2)
		set @expl='Casare ob. inv. CI '+@numar
