--***
create procedure inregPF @dataj datetime, @datas datetime, @nrdoc varchar(20)=''
as 
begin
...
@gestprim char(13),@dvi char(25),@locatie char(30),@barcod char(30),@nrpozitie int,@stare int,
...
@subunitate char(9),@PFfarainreg int,
...
exec luare_date_par 'GE','NUCPFEGAL',@PFfarainreg output,0,''
set @userASiS = isnull(dbo.fIaUtilizator(null),'')

delete from pozincon where subunitate=@subunitate and tip_document='PF' and numar_document 
	between RTRIM(@nrdoc) and RTRIM(@nrdoc)+(case when @nrdoc<>'' then '' else 'zzzzzzzzzzzzz' end) 
	and data between @dataj and @datas

declare tmpinregPF cursor for
select 
p.Subunitate,p.Tip,p.Numar,p.Cod,p.Data,p.Gestiune,p.Cantitate,p.Pret_valuta,p.Pret_de_stoc,
...
WHERE p.subunitate=@subunitate and p.tip='PF' and p.data between @dataj and @datas 
...
open tmpinregPF
fetch next from tmpinregPF into @Sub,@Tip,@Numar,@Cod,@Data,@Gest,@Cant,@Pretvaluta,@Pretstoc,
