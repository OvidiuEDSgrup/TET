/**
	Formularul este folosit pentru a lista Predari. 

**/
CREATE PROCEDURE formPredari @sesiune VARCHAR(50), @parXML XML, @numeTabelTemp VARCHAR(100)
OUTPUT AS
begin try 
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	group by pz.numar, pz.cod, pz.numar, pz.cod_intrare, pz.data
	...
	*/
	...
	convert(char(15),convert(money,round(pz.pret_de_stoc*pz.cantitate,2)),1) as PRET,
	...
	**/
	if exists (select 1 from sysobjects where type='P' and name='formPredariSP1')
	begin
		exec formPredariSP1 @sesiune=@sesiune, @parXML=@parXML, @numeTabelTemp=@numeTabelTemp output
	end
...
begin catch
	set @mesaj=ERROR_MESSAGE()+ ' (formPredari)'
