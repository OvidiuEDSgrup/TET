--***
CREATE procedure wACCodIntrare @sesiune varchar(50),@parXML XML
as
if exists(select * from sysobjects where name='wACCodIntrareSP' and type='P')
	...
	declare @Sb varchar(9), @StocCom int, @StocComLivr int, @StocLot int, @StocFurn int, 
		...
		@Iesire int, @IntrareCuMinus int, @DataSupStoc datetime, @Custodie int, @Folosinta int, @cantitate float, @SubgestFolLM int 
	...
	select	@searchText=isnull(@parXML.value('(/row/@searchText)[1]', 'varchar(80)'), ''),
			...
			@cantitate = isnull(@parXML.value('(/row/@cantitate)[1]', 'float'), '')
	...
	select @searchText=REPLACE(@searchText, ' ', '%'), 
		...
		@Folosinta=(case when @tip in ('PF', 'CI', 'AF') then 1 else 0 end)
	...
	and (@gestiune='' or s.cod_gestiune=@gestiune)
	...
	and (@tertfiltru='' or isnull(pd.tert,'')=@tertfiltru)
