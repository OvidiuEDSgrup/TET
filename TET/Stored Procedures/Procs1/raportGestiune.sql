...
as
/**
declare @datajos datetime,@datasus datetime,@gestiunea nvarchar(8),@tip_gestiune nvarchar(1)
	select @datajos='2011-03-01 00:00:00',@datasus='2011-04-01 00:00:00',@gestiunea=N'2160'
,@tip_gestiune=N'A'
--*/
...
begin try
	...
	select substring(cont_debitor,5,9) as gestiune, tip_document, numar_document, data, sum(suma) suma, 0 as sumaE, 
		max(explicatii) explicatii, 'V' as tip, 0 as val_cu_amanuntul, 0 as discount
	...
	union all 
	select substring(cont_creditor,5,9), tip_document, numar_document, data, 0, sum(suma) suma, max(explicatii) explicatii, 'V', 0, 0 
	...
	select substring(cont_corespondent,5,9), 'IN', numar, data, 0, 
			sum(suma) suma, max(explicatii) explicatii, 'V', 0, 0
			--,'', 0, 0
	from pozplin 
	...
	(case when tip='TE' then 'Gest. primitoare '+max(rtrim(p.Gestiune_primitoare)) else max(isnull(rtrim(t.denumire),'')) end) as explicatii, @q_tip_gestiune as tip, 
		sum(round(convert(decimal(17,5), cantitate*(case when @q_tip_gestiune='A' then pret_cu_amanuntul else Pret_de_stoc end)),2)) as val_cu_amanuntul, 0
	/*,space(13) as coresp
	...
	where @q_tip_gestiune<>'V' and p.subunitate=@q_sub and data between @incLuna and @q_datasus and gestiune between @q_gestiune_jos and @q_gestiune_sus 
			and (tip_miscare in ('I','E'/*,'V'*/) /*V pentru taxa verde*/ or left(cont_de_stoc,4)='4428') 
			and exists (select 1 from gestiuni where subunitate=@q_sub and tip_gestiune=@q_tip_gestiune and gestiune=cod_gestiune)
	...
	union all
	/*	AC-uri din gestiune tip C, cu gestiune primitoare completata, validata in catalogul de gestiuni si de tip A. Am tratat separat pt. a nu complica selectul de mai sus */
	/*	AP-uri din gestiune tip C, cu gestiune primitoare completata, validata in catalogul de gestiuni si de tip A, cu cantitate negativa = stornari bonuri */
	...
	max(isnull(rtrim(t.denumire),'')) as explicatii, @q_tip_gestiune as tip, 
		sum(round(convert(decimal(17,5), cantitate*pret_cu_amanuntul),2)) as val_cu_amanuntul, 
	round(sum(convert(decimal(17,5),cantitate*(case when abs(p.pret_valuta-p.pret_vanzare)<=0.01 then 0 else p.pret_valuta-p.pret_vanzare end))*(1+cota_tva/100.00)),2) as Discount
	from pozdoc p
		left join terti t on p.Tert=t.Tert and p.Subunitate=t.Subunitate
	where @q_tip_gestiune='A' and p.subunitate=@q_sub and (tip='AC' or tip='AP' and p.cantitate<0) and data between @incLuna and @q_datasus and gestiune_primitoare between @q_gestiune_jos and @q_gestiune_sus 
			and tip_miscare='E' 
	...
	union all
	/*	Am tratat mai jos si AC-uri din gestiune tip C cu gestiune primitoare completata, validata in catalogul de gestiuni si de tip A  */
	/*	AP-uri din gestiune tip C, cu gestiune primitoare completata, validata in catalogul de gestiuni si de tip A, cu cantitate negativa = stornari bonuri */
	select gestiune_primitoare, 'TI', numar, data, 
			sum(round(convert(decimal(17,5), cantitate*(case when @q_tip_gestiune='A' then (case when p.tip='TE' then pret_cu_amanuntul else convert(decimal(12,2),p.Pret_valuta*(1+p.Cota_tva/100.00)) end) 
				else Pret_de_stoc end)),2)) as suma, 0 as sumaE, 
			'Gest. predatoare '+max(rtrim(p.Gestiune)), @q_tip_gestiune
			,sum(round(convert(decimal(17,5), cantitate*(case when p.tip='AC' then p.pret_cu_amanuntul when @q_tip_gestiune='A' then pret_amanunt_predator else Pret_de_stoc end)),2)) as val_cu_amanuntul, 0
	/*		,'', 
				0, 0*/
	from pozdoc p
	where @q_tip_gestiune<>'V' and p.subunitate=@q_sub 
			...
			and p.tip_miscare='E' 
			and p.data between @incLuna and @q_datasus and p.gestiune_primitoare between @q_gestiune_jos and @q_gestiune_sus 
			and exists (select 1 from gestiuni where subunitate=@q_sub and tip_gestiune=@q_tip_gestiune and (tip='TE' or @q_tip_gestiune='A') and cod_gestiune=p.gestiune_primitoare)
			and (@fltGstUt=0 or exists (select 1 from @GestUtiliz pr where pr.valoare=p.Gestiune_primitoare))
			/*and (:9=0 or :9=1 and TVA_neexigibil<>0 or :9=2 and TVA_neexigibil=0)*/
	group by tip, gestiune_primitoare, numar, data
	/*union all
	...
	(case when tip='A' and tip_document in ('AP','AC','AS') then 'Valoare v.'+rtrim(convert(char(15),convert(decimal(12,2),sum(val_cu_amanuntul))))+
			(case when Tip_document='AC' and abs((case when sum(discount)<>0 then sum(discount) else sum(val_cu_amanuntul)-sum(sumaE) end))>0.01 
					then ' disc. '+rtrim(convert(char(15),convert(decimal(12,2),(case when sum(discount)<>0 then sum(discount) else sum(sumaE)-sum(val_cu_amanuntul) end))))
					else '' end)
	...
	where subunitate=@q_sub and data between dateadd(d,1,@q_data_inchisa) and dateadd(d,-1,@incLuna) 
		...
		and exists (select cod_gestiune from gestiuni where subunitate=@q_sub and tip_gestiune=(case when @q_tip_gestiune='A' then 'A' else 'C' end)
	...
	order by x.gestiune, data_ord, numar_document
