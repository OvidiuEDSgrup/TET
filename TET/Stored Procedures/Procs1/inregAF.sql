--***
create procedure inregAF @dataj datetime, @datas datetime, @nrdoc varchar(20)=''
as 
begin
...
delete from pozincon where subunitate=@subunitate and tip_document='AF' and numar_document 
	between RTRIM(@nrdoc) and RTRIM(@nrdoc)+(case when @nrdoc<>'' then '' else 'zzzzzzzzzzzzz' end) 
	and data between @dataj and @datas

declare tmpinregAF cursor for
...
WHERE p.subunitate=@subunitate and p.tip='AF' and p.data between @dataj and @datas 
...
open tmpinregAF
fetch next from tmpinregAF into @Sub,@Tip,@Numar,@Cod,@Data,@Gest,@Cant,@Pretvaluta,@Pretstoc,
...
begin
	...
	if 0=0
		...
		set @expl='Alte intrari fol. ob. inv. '+@numar
