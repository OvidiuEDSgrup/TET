/**
	Formularul este folosit pentru a lista Dari in folosinta. 

**/
CREATE PROCEDURE formDariFolosinta @sesiune VARCHAR(50), @parXML XML, @numeTabelTemp VARCHAR(100)
OUTPUT AS
begin try 
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	SELECT rtrim(Numar), rtrim(Cod), Data data, rtrim(Gestiune), sum(Cantitate), max(Pret_valuta), max(Pret_de_stoc), 
		...
		max(Discount), max(rtrim(pz.Tert)), rtrim(max(Factura)), max(rtrim(Gestiune_primitoare)), max(Numar_DVI), max(Valuta), 
	...
	group by pz.numar, pz.cod, pz.gestiune, pz.cod_intrare, pz.data
	...
	*/
	...
	pz.gestiune+'  '+left(g.denumire_gestiune,30) as GEST,
	rtrim(pz.cod_intrare) as CODI,
	pz.cod as COD,
	n.denumire as DENUMIRE,
	n.um as UM,
	left(convert(char(16),convert(money,round(pz.cantitate,2)),2),15) as CANT,
	convert(char(15),convert(money,round(pz.pret_de_stoc,2)),1) as PRET,
	convert(char(15),convert(money,round(pz.pret_de_stoc*pz.cantitate,2)),1) as VALOARE,
	RTRIM(pz.gestiune_primitoare) as GESTPRIM,
	convert(char(15),convert(money,round((select sum(p.pret_de_stoc*p.cantitate) from #pozdocfiltr p where p.data = pz.data and p.numar = pz.numar),2)),1) as TOTAL
	...
	**/
	if exists (select 1 from sysobjects where type='P' and name='formDariFolosintaSP1')
	begin
		exec formDariFolosintaSP1 @sesiune=@sesiune, @parXML=@parXML, @numeTabelTemp=@numeTabelTemp output
	end
...
begin catch
	set @mesaj=ERROR_MESSAGE()+ ' (formDariFolosinta)'
