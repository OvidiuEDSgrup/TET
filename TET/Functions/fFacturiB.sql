--***
--select * from dbo.fFacturiB('1',	0,'1901-01-01','2013-5-31','1000002','%','419',	0, 0, 0,0,0,0,'2011-12-31', '%','4426', '', 0)
create function  fFacturiB(@Sb varchar(9),	--> subunitate
			@IstFactImpl int,@DataJ datetime,@DataS datetime,
			--> filtre
			@Tert varchar(13),@Fact varchar(20),@cCt varchar(20),
			@Ignor4428Avans int, @4428DocFF int, @ConturiDocFF varchar(200), @GenUni int, @DocSch int,@LME int,
			@FactBil int, -- nefolosit
			@DataJMF datetime, @locm varchar(20),
			@contTvaDeductibil varchar(20), @userASiS varchar(30), @filtrareUser bit, @PrimariaTM int)
returns 
table
	/*--@docbenef 
	...
	contTVA varchar(20), cod char(20), cantitate float, contract char(20), data_platii datetime)
begin
...
select i.subunitate,i.tert,i.factura,'SI' tip ,i.factura numar,i.data,i.valoare,i.tva_11+i.tva_22 tva,i.achitat ,i.valuta, i.curs, i.valoare_valuta total_valuta, i.achitat_valuta,
		...
		'' contract, i.data as data_platii, '' efect
from istfact i
...
select p.subunitate,p.tert,p.factura,p.tip,numar,p.data,round(convert(decimal(18,5),cantitate*p.pret_vanzare),2),
		...
		0,p.loc_de_munca,left(p.comanda,20)+(case when @PrimariaTM=0 or substring(p.comanda,21,20)='' --	regula de completare a indicatorului bugetar este dupa modelul din triggerul docfac.
		...
		p.cont_factura,'2',p.cont_venituri,left(isnull(mfix.denumire, isnull(n.denumire,'Iesiri')),50),numar_pozitie,
		...
		(case when p.Tip in ('AP','AS') then p.Contract else '' end), p.data, '' efect
from pozdoc p 
...
left outer join conturi c on p.subunitate=c.subunitate and p.Cont_venituri=c.cont
left outer join mfix on isnull(n.tip, '')='F' and mfix.subunitate=p.subunitate and mfix.numar_de_inventar=p.cod_intrare
where p.subunitate=@Sb and p.tip in ('AP','AS') and p.cont_factura<>'' 
	and p.data between @DataJ and @DataS 
...
select p.subunitate,p.tert,p.factura,p.plata_incasare,p.numar,p.data,0,0,
		...
		'','','' contTVA, '' as cod,0 as cantitate, '' contract, isnull(p.detalii.value('(/row/@dataplatii)[1]','datetime'),p.Data) as data_platii,
		isnull(p.efect,'')
from pozplin p 
left join lmfiltrare pr on pr.cod=p.loc_de_munca and pr.utilizator=@userASiS
left outer join terti t on p.subunitate=t.subunitate and p.tert=t.tert 
where p.subunitate=@Sb and p.plata_incasare in ('IB','IR','PS')
	and p.data between @DataJ and @DataS 
...
left outer join terti t on p.subunitate=t.subunitate and p.tert=t.tert 
left outer join conturi cf on p.tip='FB' and p.subunitate=cf.subunitate and p.Cont_deb=cf.cont
left outer join conturi c on p.tip='FB' and p.subunitate=c.subunitate and p.cont_cred=c.cont
...
select p.subunitate,p.tert,(case when p.cod_intrare='' then 'AVANS' 
		...
		(case when @Ignor4428Avans=0 and charindex(left(p.Cont_de_stoc,3),@ConturiDocFF)='' or @4428DocFF=0 and charindex(left(p.Cont_de_stoc,3),@ConturiDocFF)<>0 then 0 else 1 end)
		...
		p.valuta,p.curs,0,(case when p.valuta<>'' and p.curs>0 then round(convert(decimal(18,5),cantitate*pret_valuta*(1-p.discount/100)),2)+
				round(convert(decimal(18,5),(case when @Ignor4428Avans=0 and charindex(left(p.Cont_de_stoc,3),@ConturiDocFF)='' or @4428DocFF=0 and charindex(left(p.Cont_de_stoc,3),@ConturiDocFF)<>0 then 0 else 1 end)
