IF EXISTS (SELECT * FROM sysobjects WHERE NAME = 'wOPFacturareContracteSP')
	DROP PROCEDURE wOPFacturareContracteSP
GO
-- procedura folosita pentru generarea de facturi din contracte/comenzi
CREATE PROCEDURE wOPFacturareContracteSP @sesiune VARCHAR(50), @parXML XML
AS
BEGIN TRY
		...
		SELECT 
			...
			(
				SELECT 
					...
					convert(DECIMAL(15, 3), (case when @cuRezervari =1 and n.tip<>'S' then prez.cantitate else p.cantitate end)) cantitate,p.pret as pvaluta,p.discount as discount,
