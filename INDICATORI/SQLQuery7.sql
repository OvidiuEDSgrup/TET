...
declare @nSumaJ float,@nSumaS float,@cCont char(13),@axe char(3000),@nel int,@i int,@j int,@pozi int,@camp char(3000),@cComanda char(3000),@nStru int  
...
begin  
  ...
  create table #ii(cod_indicator char(13),ComIns char(2000),ComUpd char(2000),Campuri char(2000))  
  ...
  while charindex('[', @sir,@poz)>0 begin  
   ...
   begin  
    ...
    if exists(select cod_indicator from indicatori where cod_indicator=@camp)  
     ...
     exec CalculCont @camp,@pDataJ,@pDataS  
    end  
   end  
