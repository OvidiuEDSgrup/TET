...
select p.tert, rtrim(t.denumire) as nume_tert,RTRIM(t.judet) as jud_tert,p.cod,rtrim(n.denumire) as nume_cod,n.um,p.cantitate,n.um_1 as um2,
	(case when isnull(n.coeficient_conversie_1,0)=0 then '' else p.cantitate/n.coeficient_conversie_1 end) as cant_um2,cant_realizata,
	c.data,p.contract as comanda,rtrim(c.explicatii) as explicatii,pret,pret*cantitate as valoare,p.Termen,
...
where (p.tip='BK' or p.tip='FC') and left(p.tip,1)=left(c.tip,1)
and p.data between @q_datajos and @q_datasus and (@q_cod is null or p.cod=@q_cod) and (@q_grupa_cod is null or n.grupa=@q_grupa_cod) and
