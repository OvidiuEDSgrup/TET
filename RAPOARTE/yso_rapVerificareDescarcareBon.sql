/*	--	procedura pentru raport de comparatie intre bonurile descarcate din bp si inregistrarile echivalente din pozdoc
	--	exemplu apel:
		declare @datajos datetime,  @datasus datetime, @necorelate bit
		select @datajos='2000-1-1', @datasus='2013-1-1', @necorelate=1

		exec yso_rapVerificareDescarcareBon @datajos=@datajos, @datasus=@datasus, @necorelate=@necorelate
*/
--***	
if exists (select 1 from sysobjects where name='yso_rapVerificareDescarcareBon')
	drop procedure yso_rapVerificareDescarcareBon
GO
--***
...
declare @datajos datetime,  @datasus datetime, @necorelate bit
select @datajos='2012-08-01', @datasus='2012-08-31', @necorelate=1
