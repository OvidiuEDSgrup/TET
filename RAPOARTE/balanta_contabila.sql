--***	Balanta contabila - procedura pentru rapoartele web Balanta contabila si Balanta contabila pe locuri de munca este rapBalantaContabilaLocm
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'balanta_contabila') AND type in (N'P'))
DROP procedure balanta_contabila
GO
--***
...
Declare @curSub char(9), @curBD char(13), @nFetch int, @eLmUtiliz int
...
declare @ContJos nvarchar(4000),@ContSus nvarchar(14),@data datetime,@valuta nvarchar(4000),@curs nvarchar(4000),@locm nvarchar(4000),@tipb nvarchar(4000),@tipBalanta int,@limba nvarchar(4000)
select @ContJos=N'371',@ContSus=N'371zzzzzzzzzzzzz',@data='2012-08-31 00:00:00',@valuta=N'',@curs=NULL,@locm=NULL,@tipb=NULL,@tipBalanta=2,@limba=NULL
/*
declare @ContJos nvarchar(4000),@ContSus nvarchar(14),@data datetime,@valuta nvarchar(4000),@curs nvarchar(4000),@locm nvarchar(4000),
		@tipb varchar(20)
select @ContJos=N'''',@ContSus=N''7zzzzzzzzzzzzz'',@data=''2010-08-20 00:00:00'',@valuta=N'''',@curs=NULL,@locm=NULL, @tipb=''2''
--*/

declare @q_ContJos varchar(13),@q_ContSus varchar(13),@q_pLuna int,@q_pAn int,@q_lb_den varchar(2),@q_valuta varchar(20), @q_curs float
	,@q_tipb varchar(20)
...
--exec rapBalantaContabilaLocm @contjos=@q_ContJos,@contsus=@q_ContSus, @pLuna=@q_pLuna, @pAn=@q_pAn, @limba=@q_lb_den, @valuta=@q_valuta, @curs=@q_curs, @cLM=@locm, @tipb=@q_tipb, @tipBalanta=@tipBalanta
exec balanta_contabila @q_ContJos,@q_ContSus,@q_pLuna,@q_pAn,@q_lb_den,@q_valuta,@q_curs, @locm, @q_tipb
