IF EXISTS (SELECT *	FROM sysobjects	WHERE NAME = 'wACTipuriDocumentSP')
	DROP PROCEDURE wACTipuriDocumentSP
GO

/** Tipuri de documente folosite la definirea plajelor **/
CREATE PROCEDURE wACTipuriDocumentSP @sesiune VARCHAR(50), @parXML XML
AS
DECLARE @searchText VARCHAR(100), @meniu VARCHAR(20)

SET @searchText = '%' + replace(ISNULL(@parXML.value('(/*/@searchText)[1]', 'varchar(100)'), ''), ' ', '%') + '%'
SET @meniu = COALESCE(--/*SP
	@parXML.value('(/*/@codMeniu)[1]', 'varchar(20)'), --SP*/
	@parXML.value('(/*/@meniupl)[1]', 'varchar(20)'),@parXML.value('(/*/@meniu)[1]', 'varchar(20)'))
select @searchText, @meniu
