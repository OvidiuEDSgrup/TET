if exists (select * from sysobjects where name ='yso_wIaPozCon')
drop procedure yso_wIaPozCon
go
--***  
create procedure [dbo].yso_wIaPozCon @sesiune varchar(50), @parXML xml      
as      
...
--SP*/ set @cautare=replace(rtrim(ltrim(@cautare)),' ','%')    
IF OBJECT_ID('tempdb..#termeneSP') IS NOT NULL  
  drop table #termeneSP  
select max(te.subunitate) as subunitate, max(te.tert) as tert, max(te.data) as data, max(te.explicatii) as explicatii, max(te.data2) as data2, max(te.termen) as termen,  
     ...
     into #termeneSP from termene te   
...
--/*startsp
		--pretvanzare=pret*(1-disc/100)/(1+case when gest.tip_gestiune in ('A','V') then p.cota_tva,0)/100)
		--suma_tva=cantitate*pretvavnzare*p.cota_tva/100
		,round(convert(decimal(17, 5),p.pret/(1+case when gp.tip_gestiune in ('A','V') then p.cota_tva else 0 end/100)
			*(1-p.discount/100)*(1-isnull(pe.pret,0)/100)*(1-isnull(pe.cantitate,0)/100)),2) as pretvanzare
		,convert(decimal(15,2),round(round(convert(decimal(17, 5), p.pret/(1+case when gp.tip_gestiune in ('A','V') then p.cota_tva else 0 end/100)
			*(1-p.discount/100)*(1-isnull(pe.pret,0)/100)*(1-isnull(pe.cantitate,0)/100)),2)
			*p.cantitate,2)) as valoare
		,convert(decimal(15,2),round(round(convert(decimal(17, 5), p.pret/(1+case when gp.tip_gestiune in ('A','V') then p.cota_tva else 0 end/100)
			*(1-p.discount/100)*(1-isnull(pe.pret,0)/100)*(1-isnull(pe.cantitate,0)/100))
			*p.Cota_TVA/100,2)*p.cantitate,2)) as valtva
		,convert(decimal(15,2),round(round(convert(decimal(17, 5), p.pret*(1-p.discount/100)*(1-isnull(pe.pret,0)/100)*(1-isnull(pe.cantitate,0)/100))
			*(1+(p.cota_tva-case when gp.tip_gestiune in ('A','V') then p.cota_tva else 0 end)/100),2)
			*p.cantitate,2)) as valtotala
--stopsp*/
into #tmppozconSP  
from pozcon p      
left outer join nomencl n on n.cod = p.Cod       
...
where p.subunitate=@sub and p.tip=@tip and p.contract=@numar and ltrim(p.tert)=@tert and p.data=@data   --/*SP
 and (@cautare='' or p.cod like '%'+@cautare+'%' or n.Denumire like '%'+@cautare+'%')  --SP*/
...
begin  
 set @areDetalii=1  
 alter table #tmppozconSP add detalii xml  
 update #tmppozconSP set #tmppozconSP.detalii=pozcon.detalii  
 from pozcon where #tmppozconSP.subunitate=pozcon.subunitate and #tmppozconSP.tip=pozcon.tip and   
     #tmppozconSP.data=pozcon.data and #tmppozconSP.numar=pozcon.contract  
end  
else  
 set @areDetalii=0  

set @doc=(select *,(select * from #tmptermenSP ter where ter.subunitate=pz.subunitate and ter.Tcontract=pz.numar and   
            ter.Tdata=pz.data and ter.cod=pz.cod for xml raw, type) from #tmppozconSP pz  
for xml raw
, root('Ierarhie')
)  
...
go
--declare @p2 xml
--set @p2=convert(xml,N'<row subunitate="1" tip="BK" numar="9820297" data="08/02/2012" explicatii="PROFORMA" termen="08/02/2012" dengestiune="DEPOZIT CJ" gestiune="212" dentert="TRUST EURO THERM  SRL - CLUJ" factura="" tert="RO6610440CJ" contractcor="" punctlivrare="" denlm="MKT-PCT LUCRU CJ" lm="1MKT20" dengestprim="AVIZE INSTALATORI PCT LUCRU" gestprim="700" valuta="" curs="0.0000" valoare="2816.04" valtva="517.79" valtotala="3333.83" scadenta="0" contclient="" procpen="0" contr_cadru="" ext_camp4="" ext_camp5="01/01/1901" ext_modificari="" ext_clauze="" valabilitate="01/01/1901" pozitii="1" discount="0.0000000e+000" comspec="0" stare="0" denstare="0-Operat" info1="" info2="0.00" info3="0.000000000000000e+000" info4="0.0000000e+000" info5="0.0000000e+000" info6="" culoare="#000000" _nemodificabil="0" _refresh="0"/>')
--exec yso_wIaPozCon @sesiune=null,@parXML=@p2