...
GO
--***
CREATE procedure [dbo].[wACFacturiSP] @sesiune varchar(50), @parXML XML  
as  
declare @subunitate varchar(9), @searchText varchar(80), @tip varchar(2), @subtip varchar(2), @tert varchar(13), 
	@valuta varchar(3), @furnbenef varchar(1), @inValuta int, @faraRestrictiiProp int, @contfact varchar(13)
...
if (rtrim(@raport)<>'' or dbo.f_arelmfiltru(dbo.fIaUtilizator(null))=0)	/**	Daca suntem pe rapoarte sau nu exista proprietatea 'LOCMUNCA' pentru utilizatorul curent se iau pur si simplu facturile*/
	...
	where f.Factura like @searchText
		...
		and (@tip<>'IF' and @subtip<>'IF' OR @contfact='' or f.Cont_de_tert=@contfact)
		and (@inValuta=0 or f.Valuta=@valuta)
		and (@tip not in ('RE', 'DE', 'EF', 'IF') or ABS(case when @inValuta=1 then f.sold_valuta else f.sold end)>=0.001)
		and (@tip<>'IF' and @subtip<>'IF' or case when @inValuta=1 then f.sold_valuta else f.sold end>=0.001)
	...
	from --facturi f
	dbo.fTert (@furnbenef, '1901-1-1', '2500-1-1', @tert, @searchText
		,CASE WHEN @tip='IF' and @subtip='IF' THEN @contfact ELSE null END
		,CASE WHEN @tip='IF' and @subtip='IF' THEN 0.001 ELSE 0 END, 0, 0, null) f
	...
	having (@tip not in ('RE', 'DE', 'EF', 'IF') or ABS(sum(case when @inValuta=1 then f.total_valuta-f.achitat_valuta else f.valoare+f.tva-f.achitat end))>=0.001)
		   and (@subtip<>'AA' or f.Factura like 'AV%')--daca este subtip de avans sa aduca numai facturile de avans
	order by 4
	for xml raw
