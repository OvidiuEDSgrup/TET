IF EXISTS (SELECT *	FROM sysobjects	WHERE NAME = 'wIaDocumenteComandaSP')
	DROP PROCEDURE wIaDocumenteComandaSP
GO

CREATE PROCEDURE wIaDocumenteComandaSP @sesiune VARCHAR(50), @parXML XML
AS
BEGIN TRY
	DECLARE 
...
--/*SP
	...
	insert #idpozc
	select pc.idPozContract, pd.idPozDoc
	from PozContracte pc join Contracte ct on ct.idContract=pc.idContract
		inner join pozdoc pd on pd.Subunitate='1' and pd.Contract=ct.numar
			and (ct.tip in ('CL') and pd.Tip in ('AC','AP','AS') or ct.tip in ('CA') and pd.Tip in ('RM','RS'))
			and pd.Cod=pc.cod
		left join #idpozc i on i.idPozDoc=pd.idPozDoc
	where ct.idContract=@idContract and i.idPozDoc is null
...
BEGIN CATCH
	SET @mesaj = ERROR_MESSAGE() + ' (wIaDocumenteComandaSP)'
