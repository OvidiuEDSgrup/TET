IF EXISTS (SELECT * FROM sysobjects WHERE NAME = 'yso_wOPPlataStornare_p')
	DROP PROCEDURE yso_wOPPlataStornare_p
GO

CREATE PROCEDURE yso_wOPPlataStornare_p @sesiune VARCHAR(50), @parXML XML
AS
...
BEGIN TRY
	...
	select @nrformular=(case when p.Cod_proprietate='UltFormGenPSAP' then rtrim(p.Valoare) else isnull(@nrformular,'') end)
	from proprietati p
	where p.Tip='PROPUTILIZ' and p.Cod=@userASIS and p.Valoare_tupla=''
		and P.Cod_proprietate in ('UltFormGenPSAP')
