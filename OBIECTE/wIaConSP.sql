---***
if exists (select * from sysobjects where name ='wIaConSP')
drop procedure wIaConSP
go
--***
create procedure wIaConSP @sesiune varchar(50), @parXML xml as  
...
select	@tip=isnull(@parXML.value('(/row/@tip)[1]','varchar(2)'),''),
		...
		,@fExplicatii='%'+replace(rtrim(ltrim(isnull(@parXML.value('(/row/@f_explicatii)[1]','varchar(100)'),''))),' ','%')+'%'
...
where d.subunitate=@subunitate 
	...
	and (@fExplicatii='' or d.Explicatii like @fExplicatii)
...
begin catch
	set @msgEroare = ERROR_MESSAGE()+'(wIaConSP)'
end catch
