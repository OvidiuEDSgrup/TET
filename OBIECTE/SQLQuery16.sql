...
--set @parXML=convert(xml,N'<row stocj="1"/>')
--set @sesiune='942F5E22F3B25'
--*** drop proc [wIaNomenclatorSP]
ALTER procedure [dbo].[wIaNomenclatorSP] @sesiune varchar(50), @parXML XML as
begin try
	if exists(select * from sysobjects where name='wIaNomenclatorSP' and type='P' and SCHEMA_NAME(uid)='yso')
		exec yso.wIaNomenclatorSP @sesiune, @parXML 
	else      
	begin
	...
	create unique nonclustered index unic on #n100 (cod)
