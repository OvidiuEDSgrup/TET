/****** Object:  StoredProcedure [dbo].[wScriuPozdoc]    Script Date: 02/13/2012 14:30:07 ******/
SET ANSI_NULLS ON
GO
...
--***
ALTER procedure [dbo].[wScriuPozdoc] @sesiune varchar(50), @parXML xml 
as
...
into (/Root/Location[@LocationID=10])[1] ');   */
		...
		set @data_expirariiS=CONVERT(char(10),@data_expirarii,101)
			...
			begin			
				...
				begin
					exec iauPozitieStoc @Cod=@cod, @TipGestiune=null, @Gestiune=null, @Data=null, 
						@CodIntrare=@codi_stoc output, @PretStoc=null, @Stoc=@stoc output, @ContStoc=null, @DataExpirarii=@data_expirarii output, 
						...
						@FltCont=null, @FltExcepCont=null, @FltDataExpirarii=@data_expirarii, @FltLocatie=@locatie, 
				...
				--- formare codiPrim pentru Transferurile directe
				
				if @tip='TE' and @TEACCODI=0 and isnull(@contract,'')=''
				begin
					declare @formNrPoz int
					exec luare_date_par 'DO','POZITIE',0,@formNrPoz output,''--alocare numar pozitie doar pt a forma codiPrim
					set @formNrPoz=@formNrPoz+1
					set @codiPrim=left(ltrim(rtrim(@codi_stoc)),12)+CHAR(65+RIGHT(@formNrPoz,1))
				END
				else 
					set @codiPrim=''
				---	
				...
				if @tip in ('AP', 'AS', 'AC')
					begin
						...
						if @tip<>'AP'
						begin
							set @pret_amanunt=(case when @tip='AS' and ISNULL(@pret_amanunt,0)>0 then @pret_amanunt else null end)
							set @pret_valuta=(case when @tip='AS' and ISNULL(@pret_valuta,0)>0 then @pret_valuta else isnull(@pret_amanunt,0)/(1.00+@cota_TVA/100) end)
						end
...
begin catch
	--ROLLBACK TRAN
	set @mesaj ='wScriuPozdoc:'+ERROR_MESSAGE()
end catch
