...
editie char(5),   
 ...
 @docInserted xml, @returneaza_inserate bit, @fara_luare_date varchar(1),@NumarDocPrimit int,@idPlajaPrimit int  
...
begin try  
 ...
 begin  
  if @cod='' and @barcod<>''  
   select top 1 @cod=Cod_produs from codbare where Cod_de_bare=@barcod     
  ...
  if @tip in ('AC','AP','AS','RM','RS') and @subtip='AA' and isnull(@cod_intrare,'')=''   
   set @cod_intrare='AV'+replace(convert(char(5),@data,103),'/','')+  
    right(convert(char(10),@data,103),2)  
...
into (/Root/Location[@LocationID=10])[1] ');   */  
  ...
  begin  
   ...
   begin  
    ...
    if @Bugetari=1 -->pentru bugetari, se scrie si indicatorul bugetar  
    begin      
     --citire date returnate in xml output de procedura de scriertare  
     set @numar_pozitie=@parXmlScriereIntrari.value('(/row/@numar_pozitie)[1]', 'int')  
     set @cont_stoc=@parXmlScriereIntrari.value('(/row/@cont_stoc)[1]', 'varchar(13)')  
       
     if left(@cont_stoc,1)='6'-->pentru receptii este nevoie de indicator bugetar doar daca contul de stoc este de clasa 6  
     begin  
      ...
      update pozdoc set comanda=@comanda_bugetari   
       where subunitate=@sub and tip=@tip and data=@data and numar=@numar and numar_pozitie=@numar_pozitie   
     end      
    end  
   end  
   ...
   IF @returneaza_inserate = 1  
    SET @docInserted = CONVERT(XML, convert(VARCHAR(max), @docInserted) + convert(VARCHAR(max), @parXmlScriereIntrari.query(  
        '/Inserate/row')))  
  end  
   ...
   begin     
    ...
    set @parXmlScriereIesiri.modify ('insert   
     ...
     begin  
      ...
      if @Bugetari=1  
      begin  
       --citire date returnate in xml output de procedura de scriere  
       set @numar_pozitie=@parXmlScriereIesiri.value('(/row/@numar_pozitie)[1]', 'int')  
       set @cont_venituri=@parXmlScriereIesiri.value('(/row/@cont_venituri)[1]', 'varchar(13)')  
       --formare indicator bugetar->atasat contului de venituri  
       exec wFormezIndicatorBugetar @Cont=@cont_venituri,@Lm=@lm,@Indbug=@indbug output   
       set @comanda_bugetari=left(@comanda,20)+@indbug   
     ...
     begin  
      ...
      begin      
       --citire date returnate in xml output de procedura de scriere  
       set @numar_pozitie=@parXmlScriereIesiri.value('(/row/@numar_pozitie)[1]', 'int')  
       set @cont_corespondent=@parXmlScriereIesiri.value('(/row/@cont_corespondent)[1]', 'varchar(13)')  
       --formare indicator bugetar->atasat contului de venituri  
       exec wFormezIndicatorBugetar @Cont=@cont_corespondent,@Lm=@lm,@Indbug=@indbug output   
       set @comanda_bugetari=left(@comanda,20)+@indbug   
       --scriere indicator bugetar in pozdoc  
       update pozdoc set comanda=@comanda_bugetari   
        where subunitate=@sub and tip=@tip and data=@data and numar=@numar and numar_pozitie=@numar_pozitie   
     ...
     IF @returneaza_inserate = 1  
     SET @docInserted = CONVERT(XML, convert(VARCHAR(max), @docInserted) + convert(VARCHAR(max), @parXmlScriereIesiri.query(  
         '/Inserate/row')))  
 ...
 /** Daca se lucreaza cu acest flag se va returna un xml de forma  
  <row idPozDoc=""/ >  
  <row idPozDoc=""/ >  
  <row idPozDoc=""/ >  
  <row idPozDoc=""/ >  
  care va contine ID-urile de PozDoc scrise in apelul curent al procedurii **/  
 IF @returneaza_inserate = 1  
  SET @parXML = @docInserted  
...
begin catch  
 ...
 if @NumarDocPrimit is not null and not exists(select 1 from docfiscalerezervate where idPlaja=@idPlajaPrimit)  
  insert into docfiscalerezervate(idPlaja,numar,expirala) values (@idPlajaPrimit,@NumarDocPrimit,getdate())  
