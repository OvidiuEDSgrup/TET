IF EXISTS (SELECT * FROM sysobjects WHERE NAME = 'wOPGenerareTransferFundamenteComanda')
	DROP PROCEDURE wOPGenerareTransferFundamenteComanda
GO

CREATE PROCEDURE wOPGenerareTransferFundamenteComanda @sesiune VARCHAR(50), @parXML XML
AS
BEGIN TRY
	...
	SET @docTransfer = 
			(SELECT @subunitate subunitate, 'TE' AS tip, convert(VARCHAR(10), @data, 101) data,@lm lm, 
					...
					@detalii as detalii, 
				(SELECT 
					p.cod AS cod, convert(decimal(17,3),(case when @cuRezervari =1 then prez.cantitate else p.cantitate end)) AS cantitate, p.idPozContract as idPozContract,
					(case when @custodie=1 then @tert else null end) as locatie, p.idPozContract idlinie,  prez.idPozDoc idpozdocrezervare
				...
				where  (@cuRezervari = 0 OR prez.idPozDoc is not null)
				FOR XML raw, type)
			FOR XML raw, type)
...
BEGIN CATCH
	SET @mesaj = ERROR_MESSAGE() + ' (wOPGenerareTransferFundamenteComanda)'
