--***
if exists (select * from sysobjects where name ='yso_wIaPretDiscount')
drop procedure yso_wIaPretDiscount
go
--***
create procedure yso_wIaPretDiscount @parXML xml, @Pret float output, @Discount float output
as
begin try
...
declare @Cod char(20), @Data datetime, @Tert char(13), @ComandaLivrare char(20), @CategPret int, 
	...
	@GrupaNom char(13), @TipCategorie int, @CategorieInValuta int, @PretReferinta float, @PretLuatInValuta int, @mesaj varchar(600)
...
begin
	...
	select top 1 
		@PretLuatInValuta=(case when @Pret is null and p.pret>0 and c.valuta<>'' then 1 else @PretLuatInValuta end), 
	...
	and (c.tip='BF' and c.stare in ('1', '3') and @Data between c.data and c.termen 
			or @ComandaLivrare<>'' and c.tip='BK' and c.contract=@ComandaLivrare)
	and (c.tip='BF' and @DiscGrupeContr=1 and @GrupaNom<>'' and left(p.mod_de_plata, 1)='G' and p.cod=@GrupaNom 
			or (c.tip='BK' or rtrim(left(p.mod_de_plata, 1))='') and p.cod=@Cod)
...
begin catch	
	set @mesaj=ERROR_MESSAGE()+ ' (yso_wIaPretDiscount)'
