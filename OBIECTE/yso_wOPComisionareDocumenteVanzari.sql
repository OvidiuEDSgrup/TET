IF EXISTS (SELECT * FROM sysobjects WHERE NAME = 'yso_wOPComisionareDocumenteVanzari')
	DROP PROCEDURE yso_wOPComisionareDocumenteVanzari
GO
CREATE PROCEDURE yso_wOPComisionareDocumenteVanzari @sesiune VARCHAR(50), @parXML XML
AS
BEGIN TRY
	...
	IF @docVanzareComisionat<>''
		...
		select @sub, @tipDoc, @dataDoc, @nrDoc, @idpozdoc
...
select * --*/ delete f
	...
	where f.idPozDoc=@idpozdoc and x.selectat=1

	DECLARE @dateInitializare XML
	SET @dateInitializare='<row idpozdoc="'+ltrim(str(@idpozdoc))+'" />'
...
BEGIN CATCH
	SET @mesaj = ERROR_MESSAGE() + ' (yso_wOPComisionareDocumenteVanzari)'
