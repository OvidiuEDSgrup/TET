...
--WHERE p.Listare='' and avnefac.terminal='ASIS'  GROUP BY avnefac.tip, avnefac.numar, avnefac.data, p.cod,con.loc_de_munca, P.TERT, TERTI.TERT, p.CONTRACT 
----having SUM(p.Cantitate)=364
--ORDER BY min(p.numar_pozitie)  
...
--go
--select * from yso_pozConExp p where p.Contract='GL980022'
--except
--select * from yso_pozConExp1 p where p.Contract='GL980022'