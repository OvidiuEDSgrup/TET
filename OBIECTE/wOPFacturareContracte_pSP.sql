IF EXISTS (SELECT * FROM sysobjects WHERE NAME = 'wOPFacturareContracte_pSP')
	DROP PROCEDURE wOPFacturareContracte_pSP
GO
CREATE PROCEDURE wOPFacturareContracte_pSP @sesiune VARCHAR(50), @parXML XML
AS
BEGIN TRY
...
BEGIN CATCH
	SET @mesaj = ERROR_MESSAGE() + ' (wOPFacturareContracte_pSP)'
