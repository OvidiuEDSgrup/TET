...
declare @Numar char(8) ,@GestPrim varchar(9),@PretAmPrim varchar(20),@CategPret int,@LM char(9),@LMdinBK char(9),@Stare int,
		@Utilizator char(10),@sub varchar(10),@tert varchar(20),@fcontract varchar(20),@fdata datetime,@gestiune varchar(20),
		...
		,@aviznefacturat bit, @nrformular varchar(10),@data_expedierii datetime, @ora_expedierii varchar(6), @iddelegat varchar(10)
...
select	@Stare=ISNULL(@parXML.value('(/*/@stare)[1]', 'varchar(20)'), ''),
		...
		@iddelegat=upper(ISNULL(@parXML.value('(/*/@iddelegat)[1]', 'varchar(10)'), '')),
		@numedelegat=upper(ISNULL(@parXML.value('(/*/@numedelegat)[1]', 'varchar(50)'), '')),
		@nrmijtransp=upper(ISNULL(@parXML.value('(/*/@nrmijtransp)[1]', 'varchar(50)'), '')),
...
begin try  
    ...
    else if @GestPrim<>'' and @Stare='1' --and (@tert<>'' or @tert='')
			  set @tip='TE'
    ...
    else if @stare in ('6') --and (@GestPrim<>'' or @gestprim='') and (@tert<>'' or @tert='') 
	...
	if @tip='TE' 
		if not exists (select 1 from pozcon p 
			...
			where p.Subunitate=@sub and p.tip='BK' and p.contract=@fcontract and 
					con.stare ='1' --and p.tert=@tert and p.data=@fdata 
		...
		if not exists (select 1 from pozcon p 
				inner join con on con.subunitate=p.subunitate and con.Contract=p.contract and con.Data=p.data and con.tert=p.tert
			where p.Subunitate=@sub and p.tip='BK' and p.contract=@fcontract and con.stare in ('1','4') --and p.tert=@tert and p.data=@fdata 
...
--/*startsp								
	...
	WHERE p.Subunitate=@Sub and p.Tip='BK' and p.Contract=@fcontract and p.Data=@fdata and p.Tert=@tert
		AND n.Tip<>'S'
		AND (s.Stoc is null 
	...
	if not exists (select Identificator from infotert i	where Subunitate='C'+@sub and tert=@tert and RTRIM(i.Descriere)=@iddelegat) and @iddelegat<>''
	begin
		declare @paramXmlString varchar(max)
		set @paramXmlString = '<row  tert=' + QUOTENAME(@tert,'""') + ' nume=' + QUOTENAME(@iddelegat,'""') + ' seriebuletin=' + QUOTENAME(@serieCI,'""') + 
				' numarbuletin=' + QUOTENAME(@numarCI,'""') + ' eliberatbuletin=' + QUOTENAME(@eliberatCI,'""') + ' />'
		if @sesiune='' select @paramXmlString
		exec wScriuPersoaneContact '', @paramXmlString
		set @idDelegat= (select MAX(Identificator) from infotert i	where Subunitate='C'+@sub and tert=@tert and RTRIM(i.Descriere)=@numeDelegat)
	end
	
	if not exists (select 1 from masinexp where Numarul_mijlocului = @nrmijtransp and Furnizor=@tert) and isnull(@nrmijtransp,'')<>''
		insert into masinexp(Numarul_mijlocului, Descriere, Furnizor, Delegat)
		values (@nrmijtransp, @nrmijtransp, @tert, '')
	...
	select tip='PROPUTILIZ', Cod=@Utilizator, Cod_proprietate='NRFORMGENDOCBK', Valoare=@nrformular into #expeditie where @nrformular<>''
...
begin tran
	...
	if @tip='TE' -- anexadoc
	    if not exists (select 1 from anexadoc where Subunitate=@sub and tip=@tip and numar=@numar and Data=@datadoc)
			...
			set Numele_delegatului=@numedelegat, Seria_buletin=@serieCI, Numar_buletin=@numarCI
				,Eliberat=@eliberatCI, Mijloc_de_transport='', Numarul_mijlocului=@nrmijtransp, Data_expedierii=@data_expedierii, Ora_expedierii=@ora_expedierii 
	    ...
	    if not exists (select 1 from anexafac where Subunitate=@sub and Numar_factura=@numar)
			...
			set Numele_delegatului=@numedelegat,Seria_buletin=@serieCI,Numar_buletin=@numarCI,Eliberat=@eliberatCI
				,Mijloc_de_transport='',Numarul_mijlocului=@nrmijtransp,Data_expedierii=@data_expedierii,Ora_expedierii=@ora_expedierii,Observatii=@observatii
	...
	begin			
		if ISNULL(@tert,'')<>''
			if not exists (select 1 from comenzi c where c.Comanda=@tert)
				...
				select top 1 t.Subunitate,'P',t.Tert,t.Denumire,t.Tert,'','',''
					,GETDATE(),GETDATE(),0,@lm,'L','',null from terti t where t.Tert=@tert
		set @input=(select top 1	rtrim(@sub) as '@subunitate',@tip as '@tip', @numar as '@numar', convert(varchar(20),@datadoc,101) as '@data',
									...
									@categpret as '@categpret', @lm as '@lm', @fcontract as '@factura',RTRIM(@tert) as '@comanda',
...
--stopsp*/									 
								...
								where p.Subunitate=@sub and p.tip='BK' and p.contract=@fcontract and (p.tert='' or p.tert=@tert)and p.data=@fdata
	...
	begin--/*sp
		...
		select @zilescadenta=nullif(c.scadenta,0) from con c where c.Subunitate=@sub and c.tip='BK' and c.contract=@fcontract 
			and (c.tert='' or c.tert=@tert )and c.data=@fdata
		...
		set @input=(select top 1	rtrim(@sub) as '@subunitate',@tip as '@tip' ,@tert as '@tert',/*sp*/@zilescadenta as '@zilescadenta',/*sp*/
...
--stopsp*/
	...
	select @nrPozBK=isnull((select count(distinct p.Cod) from pozcon p 
						left outer join con on con.subunitate=p.subunitate and con.Contract=p.contract and con.Data=p.data and con.tert=p.tert
						left outer join terti tr on tr.subunitate=p.subunitate and tr.tert=p.tert
					where p.Subunitate=@sub and p.tip='BK' and p.contract=@fcontract and (p.tert='' or p.tert=@tert )and p.data=@fdata),0),
						
			@nrPozAPsauTE=(select count(distinct cod) from pozdoc where subunitate = @sub and tip = @tip and numar = @numar and 
								data = @datadoc and (tert = @tert or @tert=''))
	/*startsp*/
	if @tip='AP'
