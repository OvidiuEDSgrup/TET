...
/*startsp
	declare @xmlStringXML xml, @iDoc int

	set @xmlStringXML=CONVERT(xml,'<row>'+@xmlString+'</row>')
	exec sp_xml_preparedocument @iDoc output, @xmlStringXML  
	
	if object_id('tempdb..#xmlStringtmp') is not null
		drop table	#xmlStringtmp
	
	select [contract],data,tert,explicatii,cod,denumire
	,cantitate
	,convert(decimal(12,3),isnull((select sum(s.stoc) from stocuri s 
		where s.Subunitate=@subunitate and s.Cod_gestiune=x.gestiune and s.Cod=x.cod and s.Stoc>0),0)) as stoc
	,um,pretcatalog,cotatva,discount,lm,gestiune,pret,valoare	 
	into #xmlStringtmp
	from openxml(@iDoc,'/row/row')  
		with ([contract] varchar(20) '@contract'
			,data date '@data'
			,tert varchar(13) '@tert'
			,explicatii varchar(max) '@explicatii'
			,cod varchar(20) '@cod'
			,denumire varchar(max) '@denumire'
			,cantitate decimal(12,3) '@cantitate'
			,um varchar(3) '@um'
			,pretcatalog decimal(17,5) '@pretcatalog'
			,cotatva int '@cotatva'
			,discount decimal(12,2) '@discount'
			,lm varchar(9) '@lm'
			,gestiune varchar(9) '@gestiune'
			,pret decimal(17,5) '@pret'
			,valoare decimal(12,2) '@valoare'
		) as x
	
	update x
	set stoc=case when stoc>=cantitate then cantitate else stoc end
	from #xmlStringtmp x
	
	delete x
	from #xmlStringtmp x where stoc<=0
	
	set @xmlString=ISNULL( 
		(select 
				...
				CONVERT(decimal(12,2),c.pretcatalog) as pretcatalog,
				c.cotatva as cotatva, discount as discount,
				-- end atribute hardcodate PV
				
				lm, --@filtru explicatii, @tert comanda_asis,
				CONVERT(decimal(12,2),c.pret) as pret,
				CONVERT(decimal(12,2),c.stoc * c.pret) as valoare,
				CONVERT(decimal(12,3),c.stoc) as stocMaxim,
				c.gestiune as gestiune
			from #xmlStringtmp c
			left outer join terti t on t.subunitate = '1' and t.tert = c.tert   
		/*	inner join preturi p on c.cod=p.Cod_produs and p.um='4' and p.Tip_pret='1' 
			...
			order by  patindex(@filtru,isnull(t.denumire, '')+c.[Contract]), c.data desc  
			for xml raw
		)+CHAR(13),'')
