--***
if exists (select * from sysobjects where name ='wACNomenclatorSP')
drop procedure wACNomenclatorSP
go
--***
create procedure wACNomenclatorSP @sesiune varchar(50),@parXML XML      
as
...
declare @nomencl table (cod varchar(20) primary key, denumire varchar(200), Cont varchar(20), Pret_cu_amanuntul float, Pret_vanzare float, UM varchar(50), stocCalculat decimal(15,2)--/*SP
, stocDepozit decimal(15,2)) --SP*/ 
insert @nomencl(cod, denumire, Cont, Pret_cu_amanuntul, Pret_vanzare, UM, stocCalculat, stocDepozit)
select top 100 
	...
	sum(convert(decimal(15, 2), isnull(stocuri.stoc, 0))) stocCalculat --/*SP
	,max(convert(decimal(15, 2), isnull(s.stoc_depozit, 0))) stocDepozit --SP*/
from nomencl
...
--/*SP
left join (select s.Cod, stoc_depozit=SUM(s.stoc) 
	from stocuri s where s.Subunitate=@subunitate and s.Tip_gestiune not in ('F','T') and s.Cod_gestiune='101' 
	group by s.Cod) s on s.Cod=nomencl.Cod
--SP*/
...
and (@tip not in ('PF','CI') or nomencl.tip='O') 
...
having (@FltStocPred=0 or nomencl.tip in ('R','S') or @tip not in ('PV','AP', 'AC', 'CM', 'TE', 'DF', 'AE', 'PF', 'CI'/*SP )*/,'BK','PV') 
		or ISNULL(@parXML.value('(/row/@codfarastoc)[1]', 'bit'), 0)=1 or max(ISNULL(s.stoc_depozit, 0))>=0.001--SP*/ 
		or sum(ISNULL(stocuri.stoc, 0))>=0.001)
...
select rtrim(nomencl.cod) as cod, 
	(case --/*SP
		...
		else (case when @tip='PV' then '('+ltrim(CONVERT(varchar(20), convert(decimal(15, 2), isnull(p.Pret_amanunt_discountat, nomencl.Pret_cu_amanuntul))))+' lei) ' else '' end)--/*SP
			+' Stoc mag= '+ --SP*/
			+ltrim(CONVERT(varchar(20), nomencl.stocCalculat))+ ' ' + rtrim(nomencl.um) --/*SP
			+', Stoc dep= '+ltrim(CONVERT(varchar(20), convert(decimal(15, 2), isnull(nomencl.stocDepozit, 0))))+ ' ' + rtrim(nomencl.um)
			--SP*/ +(case when 1=1 or @tip in ('RM','PP','AI') then ' (cont '+rtrim(nomencl.cont)+')' else '' end)
			end) as info,  
