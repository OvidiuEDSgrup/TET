...
SELECT * FROM GRUPE_VANZARE
	...
	select Subunitate, Tip, Contract, Tert, Cod, Data, discount
from pozcon p where p.subunitate='1' and p.tip='BF' --and discount<>0
and p.cod in (select g.grupa from test..grupe g)
group by Subunitate, Tip, Contract, Tert, Cod, Data, discount
having count(*)>1 
--and count(distinct discount)>1 
AND COUNT(distinct discount) over( partition by Subunitate, Tip, Contract, Tert, Cod, Data)>1

--select COUNT(distinct discount) over( partition by Subunitate, Tip, Contract, Tert, Cod, Data) from pozcon
--insert grupe
select * 
from IMPORT..grupe g1
where g1.Grupa not in (select grupa from tet..grupe)
and g1.grupa in (select p.cod from tet..pozcon p where p.Mod_de_plata='G')

select Subunitate, Tip, Contract, Tert, Cod, Data, discount, MAX(numar_pozitie)
from pozcon p1 where 
p1.subunitate='1' and p1.tip='BF' and p1.discount<>0
and p1.cod in (select g.grupa from test..grupe g)
and exists 
	(select Subunitate, Tip, Contract, Tert, Cod, Data, max(discount)
from pozcon p where p.subunitate='1' and p.tip='BF' and discount<>0
and p.cod in (select g.grupa from test..grupe g)
and p.Contract=p1.Contract and p.Cod=p1.Cod
group by Subunitate, Tip, Contract, Tert, Cod, Data
having count(*)>1 and count(distinct discount)>1 )
group by Subunitate, Tip, Contract, Tert, Cod, Data, discount
having COUNT(*)>1

select * 
delete pozcon from pozcon p2 where 
p2.subunitate='1' and p2.tip='BF' and p2.discount<>0
and p2.cod in (select g.grupa from test..grupe g)
and exists 
(select Subunitate, Tip, Contract, Tert, Cod, Data, discount, MAX(numar_pozitie)
from pozcon p1 where 
p1.subunitate='1' and p1.tip='BF' and p1.discount<>0
...
having count(*)>1 and count(distinct discount)>1 )
group by Subunitate, Tip, Contract, Tert, Cod, Data, discount
having COUNT(*)>1 and p2.Contract=p1.Contract and p2.Cod=p1.Cod and p2.Numar_pozitie=MAX(p1.numar_pozitie))


--delete pozcon from pozcon p where p.Subunitate like 'EXPAND%' and p.Tip='BF'
drop table pozcon_disc_triplu
select ROW_NUMBER() over(partition by Subunitate, Tip, Contract, Tert, Cod, Data order by discount desc) as nrcrt
--,CASE ROW_NUMBER() over(partition by Subunitate, Tip, Contract, Tert, Cod, Data order by discount desc)
--WHEN 1 THEN '1' ELSE 'EXPAND' END as sub
--,CASE ROW_NUMBER() over(partition by Subunitate, Tip, Contract, Tert, Cod, Data order by discount desc)
--WHEN 2 THEN discount ELSE pret END as pretdisc
--,CASE ROW_NUMBER() over(partition by Subunitate, Tip, Contract, Tert, Cod, Data order by discount desc)
--WHEN 3 THEN discount ELSE cantitate END as cantdisc
----,CASE ROW_NUMBER() over(partition by Subunitate, Tip, Contract, Tert, Cod, Data order by discount desc)
----WHEN 1 THEN Numar_pozitie ELSE cantitate END as cantdisc
,*
into pozcon_disc_triplu
from pozcon p1 where 
p1.subunitate='1' and p1.tip='BF' and p1.discount<>0
and p1.cod in (select g.grupa from test..grupe g)
and exists 
...
having count(*)>1 and count(distinct discount)>1
and p.Contract=p1.Contract and p.Cod=p1.Cod )
order by Subunitate, Tip, Contract, Tert, Cod, Data, discount desc

select * from pozcon p where p.Subunitate like 'EXPAND%' and p.Tip='BF' and Contract='RO14737011EU'
select * from pozcon p where p.Subunitate like '1' and p.Tip='BF' and Contract='RO14737011EU'

--delete pozcon
from pozcon p join pozcon_disc_triplu pdt on pdt.Subunitate=p.Subunitate and pdt.Tip=p.Tip and pdt.Contract=p.Contract
and pdt.Data=p.Data and pdt.Tert=p.Tert and pdt.Cod=p.Cod and pdt.Numar_pozitie=p.Numar_pozitie
where pdt.nrcrt>1
...
SELECT 'EXPAND'
      ...
      ,[Data]
      ,[Cod]
      ,isnull((select MAX(discount) from pozcon_disc_triplu pdt where pdt.Subunitate=p.Subunitate and pdt.Tip=p.Tip and pdt.Contract=p.Contract
and pdt.Data=p.Data and pdt.Tert=p.Tert and pdt.Cod=p.Cod and pdt.nrcrt=3),0)
      ,discount
      ,[Pret_promotional]
      ,[Discount]
      ,[Termen]
      ...
      ,[Explicatii]
      ,(select MAX(Numar_pozitie) from pozcon_disc_triplu pdt where pdt.Subunitate=p.Subunitate and pdt.Tip=p.Tip and pdt.Contract=p.Contract
and pdt.Data=p.Data and pdt.Tert=p.Tert and pdt.Cod=p.Cod and pdt.nrcrt=1)
      ,[Utilizator]
      ,[Data_operarii]
      ,[Ora_operarii]
from pozcon_disc_triplu p
where p.nrcrt=2

select * from pozcon_disc_triplu

--delete pozcon
where tip='BF' AND SUBUNITATE LIKE 'EXPAND%'
