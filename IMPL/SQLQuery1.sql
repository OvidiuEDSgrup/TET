...
select * from pozcon where Contract='21'
...
WHERE Subunitate = @P1 AND Contract = @P2 AND Cod = @P3 AND Data < DATEADD (day, 1, @P4) AND Stoc >= @P5    
AND (Tip_gestiune not in (''F'', ''T'')) ORDER BY Subunitate ASC ,Tip_gestiune ASC ,Cod_gestiune ASC ,Cod ASC ,Data ASC ,Cod_intrare ASC '
...
exec sp_executesql N'SELECT Subunitate,Tip_gestiune,Cod_gestiune,Locatie,Contract,Lot,Data_expirarii,Cod,Data,Stoc,Cod_intrare,Pret,Cont,TVA_neexigibil,Pret_cu_amanuntul FROM TET..stocuri 
WHERE Subunitate = @P1 AND Tip_gestiune = @P2 AND Cod_gestiune = @P3 AND Contract = @P4 AND Cod = @P5 AND Data < DATEADD (day, 1, @P6) AND Stoc >= @P7    AND (Tip_gestiune not in (''F'', ''T'')) ORDER BY Subunitate ASC ,Tip_gestiune ASC ,Cod_gestiune ASC ,Cod ASC ,Data ASC ,Cod_intrare ASC '
