...
select '103', '02/02/2012', 'IT08010000000',p.cod, 'BK', 
...
insert into pozaprov
(Contract, Data, Furnizor, Cod, Tip, Comanda_livrare, Data_comenzii, Beneficiar, Cant_comandata, Cant_receptionata, Cant_realizata)
select '103', '02/02/2012', 'IT08010000000',
...
delete from pozaprov where contract='103' and data='02/02/2012' and furnizor='IT08010000000' and cant_comandata < 0.001
...
(case when 1  not in (0, 1) then 0 else isnull((select sum(pa.cant_comandata-pa.cant_receptionata) from pozaprov pa inner join pozcon g 
   ...
   where g.tip='FC' and (1=0 OR 0=0 or g.factura='101') and g.cod=n.cod and pa.furnizor='IT08010000000' and 
...
(case when 1  = 0 then '02/02/2012' when 1  = 1 then
isnull((select dateadd(day,0*(-1),min(termen)) from pozcon pc where pc.cod = n.cod and pc.tip = 'BK' 
and pc.cant_aprobata-pc.cant_realizata>=0.001 and pc.contract in (select comanda_livrare from pozaprov where tip='BK' and contract = '103')),'02/02/2012')
else '02/02/2012' end), 'OVIDIU', 
--comenzi de productie 
0 as Com_interne
from nomencl n
inner join tempdb..temp_cod_furn7964 pp on pp.cod = n.cod
left outer join stoclim sl on n.cod=sl.cod and ((1=1 and sl.cod_gestiune='101') or (1=0 and sl.cod_gestiune='')) and sl.data<='01/01/2999'
where n.tip in ('A', 'M') and (0=0 or n.cod like RTrim('')) and (charindex(','+rtrim(n.grupa)+',','') > 0 or 0 = 0)
group by n.cod, pp.furnizor

if 1  = 2 update comaprovtmp set termen = dateadd(day,(select isnull(max(p.nr_zile_livrare),0) from ppreturi p where comaprovtmp.cod = p.cod_resursa and comaprovtmp.furnizor = p.tert),termen)

update comaprovtmp
set com_clienti = isnull((select sum(cant_comandata) from pozaprov p where p.contract='103' and p.data='02/02/2012' and p.furnizor='IT08010000000' and p.cod=comaprovtmp.cod and p.tip='BK' and p.comanda_livrare<>'' group by p.cod), 0),
com_interne = isnull((select sum(cant_comandata) from pozaprov p where p.contract='103' and p.data='02/02/2012' and p.furnizor='IT08010000000' and p.cod=comaprovtmp.cod and p.tip in ('C', 'N') and p.comanda_livrare<>'' group by p.cod), 0)
...
delete from pozaprov where contract='103' and data='02/02/2012' and furnizor='IT08010000000' and cod not in (select distinct cod from comaprovtmp where utilizator='OVIDIU')
...
insert into pozaprov
(Contract, Data, Furnizor, Cod, Tip, Comanda_livrare, Data_comenzii, Beneficiar, Cant_comandata, Cant_receptionata, Cant_realizata)
select '103', '02/02/2012', 'IT08010000000', cod, '', '', '02/02/2012', '', 
...
delete from pozaprov where contract='103' and data='02/02/2012' and furnizor='IT08010000000' and cod in (select cod from comaprovtmp where utilizator='OVIDIU' group by cod having sum(de_aprovizionat)<0.001) and comanda_livrare <> ''
delete from pozaprov where contract='103' and data='02/02/2012' and furnizor='IT08010000000' and tip='' and comanda_livrare='' and abs(cant_comandata)<0.001 
 and exists (select 1 from pozaprov p1 where p1.contract='103' and p1.data='02/02/2012' and p1.furnizor='IT08010000000' and p1.cod=pozaprov.cod and p1.cant_comandata>=0.001)

delete from comaprovtmp where 1  in (1,2) and total=0 and com_clienti=0 and com_interne=0 and stoc_limita=0 and utilizator='OVIDIU'
