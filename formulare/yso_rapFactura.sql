if exists (select 1 from sys.sysobjects where name = 'yso_rapFactura' and type = 'P')
	drop procedure yso_rapFactura
go
	/*
	Formular in lei
	...
	EXEMPLU APEL 
		exec yso_rapFactura @sesiune='',@tip='AS', @numar='11000129', @data='2016-01-27', @nrexemplare=1
	*/
create procedure yso_rapFactura (@sesiune varchar(50)=null, @tip varchar(2), @numar varchar(20), @data datetime, @nrExemplare int=1,
...
begin try
	...
	*/
	...
	create table #PozDocFiltr (tip varchar(2), numar varchar(20), data datetime, tert varchar(50),
		...
		locm varchar(50) default null, comanda varchar(50), gestiune varchar(20), gestiune_primitoare varchar(20), data_facturii datetime, data_scadentei datetime, nraviz varchar(20), serie varchar(20))
	...
	insert into #PozDocFiltr (tip, numar, data, tert, factura, cod, denumire, um, cantitate, pret, valoare, pret_valuta, curs, cota_tva, pret_vanzare, tva_deductibil, tva_unitar, 
		discount_unitar, discount, subtip, numeutilizator, cnputilizator, locm, comanda, valuta, gestiune, gestiune_primitoare, data_facturii, data_scadentei,serie)
	select
		max(p.tip),
		p.numar, 
		p.data,
		...
		p.pret_valuta*(case when max(p.curs)>0 then max(p.curs) else 1 end)*max(p.cota_tva)/100,
		...
		,i.lot	
	from PozDoc p 
	left join pozdoc i on i.idPozDoc=p.idIntrareFirma
	...
	where p.cont_de_stoc not like '418%'
		and (@Doar_cu_serie=0 or isnull(i.lot,'')<>'')
	group by p.numar, p.data, p.tert, p.factura, p.cod, p.pret_vanzare, p.pret_valuta, p.Discount, p.Comanda, i.lot
	...
	*/
	insert into #PozDocFiltr (tip, numar, data, tert, factura, cod, denumire, um, cantitate, pret, valoare, pret_valuta, curs, cota_tva, pret_vanzare, tva_deductibil, tva_unitar, 
		discount_unitar, discount, subtip, numeutilizator, cnputilizator, locm, comanda, valuta, gestiune, gestiune_primitoare, data_facturii, data_scadentei, nraviz, serie)
	select
		intoc.tip,
		rtrim(intoc.numar), 
		intoc.data,
		...
		,i.lot
	from PozDoc intoc
	left join pozdoc i on i.idPozDoc=intoc.idIntrareFirma
