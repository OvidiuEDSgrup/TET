...
drop procedure yso_xIaTerti 
go
create procedure yso_xIaTerti as 
select * from yso_vIaTerti
go
if exists (select * from sysobjects where name ='yso_xImportTerti')
drop procedure yso_xImportTerti
go
...
--begin try--*/declare @tabela varchar(255)='stoclim', @fisier nvarchar(4000)='\\10.0.0.10\import\testimport.xlsx' 
	...
	from yso_vIaStoclim
GO
if exists (select * from sysobjects where name ='yso_xScriuTerti')
drop procedure yso_xScriuTerti
go
