...
--/*
if exists (select * from sysobjects where name ='yso_xStergTabela')
drop procedure yso_xStergTabela
go
create procedure yso_xStergTabela @tabela varchar(255), @sursaImport nvarchar(4000) as
...
--sp*/
	...
	set @txtSql='select * into ##importXlsIniTmp from '+@txtFrom
...
*/
	...
	begin
		begin try
			select @txtSql='yso_xSterg'+@tabela+' @_nrdif', @txtParam='@_nrdif int'
...
begin catch
	...
	set @mesaj = 'yso_xStergTabela: '+ ERROR_MESSAGE() 
	raiserror(@mesaj, 11, 1)	
end catch
go
/*
delete m from mesajeASiS m where m.Destinatar=HOST_ID()
exec yso_xStergTabela 'pozcon', '\\10.0.0.10\import\testimport.xlsx'
