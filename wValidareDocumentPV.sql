--***
--***
alter procedure yso.wValidareDocumentPVSP @sesiune varchar(50), @parXML XML
as
--declare @returnValue int
--if exists(select * from sysobjects where name='wValidareDocumentPVSP' and type='P' and SCHEMA_NAME(uid)='yso')      
--begin
--	exec @returnValue = yso.wValidareDocumentPVSP @sesiune,@parXML
--	return @returnValue 
--end
...
begin try
	...
	from #stocuri st inner join 
		(select s.subunitate,s.cod, sum(s.stoc) as stoc from stocuri s 
			inner join dbo.split(@listagestiuni,';') lg on s.Cod_gestiune=lg.Item group by s.subunitate,s.cod) as s 
	on s.Subunitate=@subunitate and s.Cod=st.cod
	where s.cod=st.cod
