 ...
 if @tip='TE'   
  if not exists (select 1 from pozcon p   
   ...
   where p.Subunitate=@sub and p.tip='BK' and p.contract=@fcontract and   
    ...
    and abs(p.Cant_aprobata-p.Pret_promotional)>0.001) -- Pret_promotional este refolosit pt. cant. transferata   
...
--/*startsp          
 ...
 select @msgErr=isnull(@msgErr+CHAR(13),'')+RTRIM(max(n.denumire))+' ('+RTRIM(p.Cod)+')'  
   +', lipsa: '+ rtrim(CONVERT(decimal(10,2),MAX(p.Cant_aprobata)-SUM(isnull(s.Stoc,0))))  
 FROM pozcon p   
 ...
 GROUP BY p.Cod  
 having SUM(isnull(s.Stoc,0))<MAX(p.Cant_aprobata)-max(p.Cant_realizata)  
  ...
  *(p.Cant_aprobata-(case when @tip='TE' then p.Pret_promotional else p.cant_realizata end)),2))  
 from pozcon p   
...
--stopsp*/            
        ...
        and abs(p.cant_aprobata-(case when @tip='TE' then p.Pret_promotional else p.cant_realizata end))>0.001  
