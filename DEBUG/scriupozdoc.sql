declare @parXML xml
set @parXML=convert(xml,N'<row numar="" data="05/11/2012" gestiune="400.3" comanda="" gestprim="213" categpret="" tip="TE"><row cod="25-ISO4-26-BL" cantitate="50" pamanunt="0" discount="0" subtip="TE"/></row>')
exec wScriuPozdoc @sesiune='87CF8721C48E5',@parXML=@parXML
declare @iDoc int
	EXEC sp_xml_preparedocument @iDoc OUTPUT, @parXML
	
	--declare crspozdoc cursor for
	select *,
	tip, upper(numar), data, 
	upper((case when isnull(gestiune_pozitii, '')<>'' then gestiune_pozitii when isnull(gestiune_antet, '')<>'' then gestiune_antet else '' end)) as gestiune, 
	...
	--isnull(cont_de_storc, '') as cont_stoc, isnull(pret_de_stoc, 0) as pret_stoc, 
	...
	0--/*isnull(*/cont_intermediar/*, '')*/ as cont_intermediar, 
