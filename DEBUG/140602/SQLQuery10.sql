...
/****** Object:  StoredProcedure [dbo].[raportGestiune]    Script Date: 06/02/2014 12:00:29 ******/
SET ANSI_NULLS ON
GO
...
--***
create procedure [dbo].[raportGestiune] (@datajos datetime,@datasus datetime, @gestiunea varchar(20),@tip_gestiune nvarchar(1), @soldc int=0)
as
/**
...
begin try
	...
	from pozdoc p
		left join terti t on p.Tert=t.Tert and p.Subunitate=t.Subunitate
	where @q_tip_gestiune='A' and p.subunitate=@q_sub and tip='AC' and data between @incLuna and @q_datasus and gestiune_primitoare between @q_gestiune_jos and @q_gestiune_sus 
			and tip_miscare='E' 
	...
	select gestiune_primitoare, 'TI', numar, data, sum(round(convert(decimal(17,5), cantitate*
			...
			,sum(round(convert(decimal(17,5), cantitate*(case when p.tip='AC' then p.pret_cu_amanuntul when @q_tip_gestiune='A' then pret_amanunt_predator else Pret_de_stoc end)),2))
	/*		,'', 
				0, 0*/
	from pozdoc p
	where @q_tip_gestiune<>'V' and subunitate=@q_sub and (tip='TE' or tip='AC' and exists(select 1 from gestiuni where subunitate=@q_sub and tip_gestiune='C' and cod_gestiune=gestiune)) 
			and data between @incLuna and @q_datasus and gestiune_primitoare between @q_gestiune_jos and @q_gestiune_sus 
			and exists(select 1 from gestiuni where subunitate=@q_sub and tip_gestiune=@q_tip_gestiune and (tip='TE' or @q_tip_gestiune='A') and cod_gestiune=gestiune_primitoare)
			and (@fltGstUt=0 or exists(select 1 from @GestUtiliz pr where pr.valoare=p.Gestiune_primitoare))
			/*and (:9=0 or :9=1 and TVA_neexigibil<>0 or :9=2 and TVA_neexigibil=0)*/
	group by tip, gestiune_primitoare, numar, data
	/*union all
