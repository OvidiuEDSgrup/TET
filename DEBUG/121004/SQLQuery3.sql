--***  
create procedure wDescarcBon @sesiune varchar(50), @parXML xml  
as  
...
declare @nFetchStatus int,@subunitate varchar(9),@Serii int,   
 ...
 @gestPozitie varchar(30), @listaGestiuniPozitie varchar(300), @tertNeplatitorTva bit,  
 @rezervareStocComenzi bit, @gestRezervariComenzi varchar(13), @DetaliereBonuri bit, @tmpbon cursor, @CuTranzactii int, @total float  
...
select   
 ...
 (CASE WHEN @tertNeplatitorTva=0 THEN 0 ELSE convert(INT, left(n.tip_echipament, 1)) END) AS tipTVA,  
...
begin  
 if DATEDIFF(second, @dataStart, getdate()) >= 25 and APP_NAME() not like '%Microsoft SQL Server%'   
  ...
  begin  
    ...
    */  
    ...
    /*  
     ...
     daca se gaseste un cod intrare valid, se storneaza pe acelasi cod de intrare; altfel,  
     se genereaza un cod intrare nou  
     la facturi si transferuri, orice cod de intrare e valid; la bonuri, codul de intrare trebuie  
     sa aiba acelasi pret cu amanuntul pentru a fi valid.  
    */  
       ...
       (select top 1 1 as '@fara_luare_date', rtrim(@subunitate) as '@subunitate', 'TE' as '@tip',   
        ...
        @GestStPrim as '@gestprim', 5 as '@stare',  
         (select rtrim(@Cod) as '@cod', convert(decimal(14,5),-1*@Stoc) as '@cantitate',   
          ...
          @contract as '@factura', @comanda_asis as '@comanda'  
          for xml PATH, TYPE)  
        for XML PATH, type)  
    ...
    */  
    ...
    begin  
     ...
     if @TipDoc='AC' and @comanda_asis='' and @contract='' and   
      exists(select 1 from pozdoc where subunitate=@subunitate and tip=@tipDoc and numar=@NrDoc and data=@data and cod=@cod and   
       cod_intrare=@codISt and pret_vanzare=@pretVanzareDeScris  
       and Discount=@Disc and Loc_de_munca=@LM and Utilizator=@Vanz)  
     begin -- cumulare pozitii doar la AC-uri la care nu se vinde din deviz sau comanda de livrare.  
      update pozdoc set cantitate=cantitate+@cantDeScris,tva_deductibil=tva_deductibil+@TvaDeScris  
       where subunitate=@subunitate and tip=@tipDoc and numar=@NrDoc and data=@data and cod=@cod and cod_intrare=@codISt and pret_vanzare=@pretVanzareDeScris  
     end  
     else --Pozitie noua  
     begin  
      set @xml=(select @tipDoc as tip, '' as subtip, @NrDoc as numar, convert(char(10), @data,101) as data,  
...
begin catch  
 set @msgEroare = ERROR_MESSAGE()+'. (idAntetBon='+convert(varchar,@idantetbon)+') (wDescarcBon)'  
