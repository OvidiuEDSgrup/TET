drop procedure [dbo].[wDescarcBon] 
go
--***  
...
begin  
  ...
  */  
  ...
  begin  
   ...
   begin  
    ...
    and charindex(';'+RTrim(s.cod_gestiune)+';',@listaGestiuniStorno)>0                                                                                                                                                                                                                    dxc8te9kn0
    and (s.Tip_gestiune<>'A' or abs(s.Pret_cu_amanuntul-@Pret)<0.0009)  
     ...
     (select top 1 rtrim(@subunitate) as '@subunitate', @TipDoc as '@tip', rtrim(@Client) as '@tert',  
      ...
      @LM as '@lm',@GestSt as '@gestiune',  @contract as '@contract', 5 as '@stare',  
       (select rtrim(@Cod) as '@cod', convert(decimal(14,5),@Stoc) as '@cantitate',   
        ...
        convert(decimal(14,5),@PretAm) as '@pamanunt', convert(decimal(14,5),@Disc) as '@discount',
 ...
 fetch next from @tmpbon into @Incas,@Casa,@Data,@NrBon,@Vanz,@NrLin,@TipDoc,@Client,@Cod,@AreSerii,@Serie,@Coef,@Cant,@CotaTVA,@SumaTVA,@Pret,@Disc,@Barcod,@TipNom,@LM,@comanda_asis,@dataScad,@CategP,@PctLiv,@contract,@Jurn,@AP418,@factura,@bTip, @tipTVA, @gestPozitie, @UID, @idAntetBon  
 set @nFetchStatus=@@fetch_status  
