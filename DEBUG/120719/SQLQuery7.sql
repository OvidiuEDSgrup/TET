drop procedure [dbo].[wValidareDocumentPVSP1] 
go
...
begin try  
 ...
 -- insert #stocurisp1(cod, tipnom, cantitate, stoc, gestiune)   
 ...
 -- update st   
 --  set st.stoc=isnull(st.stoc,0)+/*startsp*/(CASE WHEN s.stoc<0 THEN 0 ELSE s.stoc END)/*stopsp*/  
 --  from #stocurisp1 st  
 --  inner join   
 ...
 -- if exists(select 1 from #stocurisp1 where gestiune<>@GESTPV and stoc<cantitate)  
 -- begin  
 --  update st   
 --   set st.stoc=isnull(st.stoc,0)+/*startsp*/(CASE WHEN s.stoc<0 THEN 0 ELSE s.stoc END)/*stopsp*/  
 --   from #stocurisp1 st  
 ...
 --     from stocuri s  
 --     inner join #stocurisp1 stocTmp on Cod_gestiune=stocTmp.gestiune and s.cod=stocTmp.cod  
 --     where Subunitate=@subunitate  
 --     group by s.cod) s on s.cod=st.cod  
 -- end  
    
  -- verific daca sunt produse la care stocul ar fi insuficient  
 -- if exists (select 1 from #stocurisp1 where tipnom<>'S' and stoc<cantitate)  
 -- begin  
 --  set @ErrorMessage='Stoc insuficient pentru:'  
 --  select @ErrorMessage=@ErrorMessage+CHAR(13)+RTRIM(Denumire)+' (max. '+convert(varchar(30),convert(decimal(12,3),#stocurisp1.stoc))+' '+nomencl.UM+')'  
 --  from #stocurisp1   
 --  inner join nomencl on #stocurisp1.cod=nomencl.cod  
 --  where #stocurisp1.stoc<cantitate  
